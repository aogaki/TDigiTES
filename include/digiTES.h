...
#define WAIT_ON_EXIT \
  10000  // 10 sec before closing the console (only in case of error)
...
#ifndef CH2KEV
#define CH2KEV(e, c0, c1, c2, c3) \
  ((e) * (e) * (e) * (c3) + (e) * (e) * (c2) + (e) * (c1) + (c0))
#endif
...
// Data File options
//****************************************************************************
#define DATA_FILE_FORMAT \
  0  // File format (must be changed when data file format or structures size change)

//****************************************************************************
// Event data Flags
//****************************************************************************
#define EVFLAGS_DEADTIME \
  0x0001  // dead time (memory full or input saturation) occurred between this event and the previous one
#define EVFLAGS_SATUR \
  0x0002  // input voltage saturation (clipping)  HACK: can indicate also energy overrange?
...
#define EVFLAGS_UNCORREL \
  0x0800  // Uncorrelated Event (tagged instead of discarded)
...
#define ACQ_MODE_UNPLUGGED(x)                                  \
  ((x == ACQ_MODE_OFFLINE) || (x == ACQ_MODE_EMULATOR_LIST) || \
   (x == ACQ_MODE_EMULATOR_MIXED))
...
#define COINC_DISABLED \
  0x00  // no hardware trigger logic is applied (acquire singles)
#define COINC_MAJORITY \
  0x01  // acquire when the num of fired channels >= Majority Threshold
#define COINC_PAIRED_AND \
  0x02  // both channels of a couple acquire when both are fired
#define COINC_CH0_AND_ANY \
  0x03  // coincidence between a common ref channel (typ. ch 0) and any other channel
#define COINC_AND_ALL 0x04  // acquire when all enabled channels are fired
#define COINC_MINORITY \
  0x81  // acquire when the num of fired channels < Majority Threshold
#define COINC_OR_ALL \
  0x0A  // all enabled channels acquire if any channel is fired (trigger propagation from any channel to all)
#define COINC_CH0_TO_ALL \
  0x0B  // all enabled channels acquire if channel 0 is fired (trigger propagation from channel 0 to all)
#define COINC_PAIRED_OR \
  0x0C  // both channels of a couple acquire when one at least on of the two is fired (trigger propagation in the couple)
#define COINC_PAIRED_NAND \
  0x82  // one channel of a couple acquires when the other one is not fired (anti coincidence)
#define COINC_CH0_NAND_ANY \
  0x83  // any channel acquires is there is no coincidence with the ref. channel

// options for the event building
#define EVBUILD_NONE 0  // No correlation
#define EVBUILD_CHREF_AND_ANYOTHER \
  1  // One reference channel in coincidence with any N channels, N>=1
#define EVBUILD_PAIRED_AND \
  2  // Couples with coincidence between ch N and ch N+1
#define EVBUILD_CLOVER \
  3  // at least one channel between N (typ N=4); no more than 1 event per channel
...
typedef struct {
  ...
  uint16_t *AnalogTrace
      [2];  // Analog traces (samples); the 2nd trace is enabled if DualTrace=1
...
typedef struct {
  ...
  uint16_t
      Energy;  // Energy (charge for DPP_PSD/CI or pulse height for DPP_PHA)
  float
      psd;  // Pulse Shape Discrimination (PSD = (Qlong-Qshort)/Qlong). Only for DPP_PSD. Range 0 to 1023.
  Waveform_t *
      Waveforms;  // Pointer to waveform data (NULL if the waveform mode is not enabled)
...
typedef struct Config_t {
  ...
  uint32_t EnableMask
      [MAX_NBRD];  // Channel enable mask (bit n => channel n) for each board
  ...
  char DataFilePath
      [200];  // path to the folder where output data files are written
  char InputDataFilePath
      [200];  // path to the folder where input data files are read (if not set, DataFilePath is used)
  char InputDataFileName
      [200];          // name of the input data file for the off-line run
  ...
  int NsBaseline
      [MAX_NBRD]
      [MAX_NCH];  // Num of Samples for the input baseline calculation (for PSD)
  int TrapNSBaseline
      [MAX_NBRD]
      [MAX_NCH];  // Num of Samples for the trapezoid baseline calculation (for PHA)
  int FixedBaseline[MAX_NBRD][MAX_NCH];  // Fixed baseline when NsBaseline=0
  int GateWidth
      [MAX_NBRD]
      [MAX_NCH];  // Gate Width (in ns) (it's the long gate for DPP_PSD)
  int ShortGateWidth[MAX_NBRD][MAX_NCH];  // Short Gate Width (in ns)
  int PreGate
      [MAX_NBRD]
      [MAX_NCH];  // Pre Gate (num of samples the gate starts before the trigger)
  int ChargeSensitivity
      [MAX_NBRD][MAX_NCH];  // Charge sensitivity (value depends on the FW type)
  int ChargeLLD[MAX_NBRD]
               [MAX_NCH];  // Charge Cut (low level discr based on charge)
  ...
  int PileUpMode
      [MAX_NBRD]
      [MAX_NCH];  // Pile Up mode: 0=ignore, 1=reject, 2=retrigger (x751 only)
  int DCoffset[MAX_NBRD][MAX_NCH];  // input DC offset (from 0 to 65535)
  int ACcoupling
      [MAX_NBRD]
      [MAX_NCH];  // enable AC coupling (Hexagon only). 0=disabled, 1=1.8us, 2=4.7us, 3=10us
  int ACPZcomp[MAX_NBRD]
              [MAX_NCH];  // Pole Zero compensation for the AC coupling
  float BaselineDCoffset
      [MAX_NBRD][MAX_NCH];  // another way to set the input DC offset (0 to 100)
  float PSDcutThr[MAX_NBRD]
                 [MAX_NCH];  // on board PSD cut from 0 to 1 (0=disabled)
  int PSDcutType
      [MAX_NBRD]
      [MAX_NCH];  // 0=cut gammas (cut if PSD<thr), 1=cut neutrons (cut if PSD>thr)
  int EnablePedestal
      [MAX_NBRD]
      [MAX_NCH];  // Enable Charge pedestal (add a fixed offset of 1024 to the charge value)
  int CFDdelay[MAX_NBRD][MAX_NCH];  // CFD delay in sampling periods
  int CFDfraction[MAX_NBRD]
                 [MAX_NCH];  // CFD fraction 0=25%, 1=50%, 2=75%, 3=100%
  int CFDinterp
      [MAX_NBRD]
      [MAX_NCH];  // Interpolation Mode 0=ZC+/-1, 1=ZC+/-2, 2=ZC+/-3 2=ZC+/-4
  int DiscrMode
      [MAX_NBRD]
      [MAX_NCH];  // Discriminator Mode: 0=LED or RCCR2, 1=CFD, 2=CFD_PHS 3=LED_PHA
  int TrapRiseTime[MAX_NBRD][MAX_NCH];  // Trapezoid Rise Time (DPP_PHA)
  int TrapFlatTop[MAX_NBRD][MAX_NCH];   // Trapezoid Flat Top (DPP_PHA)
  int TrapPoleZero
      [MAX_NBRD]
      [MAX_NCH];  // Trapezoid Pole-Zero cancellation (input exponential decay time) (DPP_PHA)
  ...
  int NSPeak
      [MAX_NBRD]
      [MAX_NCH];  // Numer of samples averaged in the flat top to get the peak value = energy (DPP_PHA)
  int PeakHoldOff
      [MAX_NBRD]
      [MAX_NCH];  // Minimum distance between Trapezoids (below this, the PUR will discard them) (DPP_PHA)
  int TTFsmoothing
      [MAX_NBRD]
      [MAX_NCH];  // Smoothing factor in the trigger and timing filter (DPP_PHA)
  int SmoothedForCharge
      [MAX_NBRD][MAX_NCH];  // Use smoothed input for the charge integration
  int TTFdelay[MAX_NBRD]
              [MAX_NCH];  // Delay in the trigger and timing filter (DPP_PHA)
  int Decimation
      [MAX_NBRD]
      [MAX_NCH];  // Decimation 0=disabled, 1=1/2, 2=1/4, 3 = 1/8 (DPP_PHA)
  ...
  int EnableIPE
      [MAX_NBRD]
      [MAX_NCH];  // Enable/Disable Internal pulse emulator (replacing real data from input ADC)
  int IPEfrequency
      [MAX_NBRD]
      [MAX_NCH];  // IPE frequency: 0=100Hz, 1=1KHz, 2=10KHz, 3=100KHz (approx.)
  int IPEdecay[MAX_NBRD][MAX_NCH];  // IPE decay time in us:
  int IPEaddnoise[MAX_NBRD]
                 [MAX_NCH];  // IPE add random noise (0=no noise, 1=noise added)
  int IPEamplitude[MAX_NBRD][MAX_NCH];  // IPE pulse amplitude in ADC LSB
  int IPErisetime
      [MAX_NBRD]
      [MAX_NCH];  // IPE rise time (low pass filter); 0 to 7, 0 is the fastest
  int IPErandom[MAX_NBRD]
               [MAX_NCH];  // IPE random mode: 0=periodic, 1=pseudo poissonian
  ...
  int ZeroVoltLevel
      [MAX_NBRD]
      [MAX_NCH];  // Position of the baseline (zero volt) in ADC channels

  // Energy Gain and Calibration
  float EnergyCoarseGain
      [MAX_NBRD]
      [MAX_NCH];  // Energy Coarse Gain (requested by the user); can be a power of two (1, 2, 4, 8...) or a fraction (0.5, 0.25, 0.125...)
  float EnergyFineGain[MAX_NBRD]
                      [MAX_NCH];  // Energy Fine Gain (requested by the user)
  uint16_t EnergyDiv
      [MAX_NBRD]
      [MAX_NCH];  // Energy Divisor to get the requested CoarseGain and Rebinning
  float ECalibration_c0
      [MAX_NBRD]
      [MAX_NCH];  // Energy Calibration c0, c1, c2, c3 (Ekev = Ech*c3^3 + Ech*c2^2 + Ech*c1 + c0)
  ...
  float TstampOffset
      [MAX_NBRD]
      [MAX_NCH];      // Delay line in ns (add a delay to each time stamps)
  ...
  // ----------------------------------------------------------------------
  float EnergyLCut[MAX_NBRD]
                  [MAX_NCH];  // lower Level energy cut for the event selection
  float EnergyUCut[MAX_NBRD]
                  [MAX_NCH];  // upper Level energy cut for the event selection
  float PsdLCut[MAX_NBRD]
               [MAX_NCH];  // lower Level PSD cut for the event selection
  float PsdUCut[MAX_NBRD]
               [MAX_NCH];  // lower Level PSD cut for the event selection
  float
      TimeCorrelWindow;  // Time interval for two events to be considered correlated
  ...
  float THmin[MAX_NBRD]
             [MAX_NCH];  // lower time value used to make time histograms
