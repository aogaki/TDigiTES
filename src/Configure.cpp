...
int ProgramDigitizer(int brd, Config_t &WDcfg, const int handle[])
{
  ...
  if ((WDcfg.DppType == DPP_PHA_724) || (WDcfg.DppType == DPP_nPHA_724)) {
    WDcfg.Tsampl *=
        (1 << WDcfg.Decimation[0]
                              [0]);  // HACK: assuming the same for all channels
  ...
  if (WDcfg.DigitizerModel != 5000)
    ret |= CAEN_DGTZ_WriteRegister(handle[brd], 0x8168,
                                   (WDcfg.FanSpeed << 3) & 0x8);

  if (WDcfg.DigitizerModel == 5000)
    ret |= CAEN_DGTZ_WriteRegister(
        handle[brd], 0x8000,
        0);  // Channel Control Reg (indiv trg, seq readout) ??
  else if ((WDcfg.DppType == DPP_PHA_724) || (WDcfg.DppType == DPP_nPHA_724))
    ret |= CAEN_DGTZ_WriteRegister(
        handle[brd], 0x8000,
        0x01000114);  // Channel Control Reg (indiv trg, seq readout) ??

  if (WDcfg.DigitizerModel == 751)
    ret |= RegisterSetBits(handle[brd], 0x8000, 31, 31, 1,
                           WDcfg);  // Enable new format

  // settings for DPP firmware only
  if (IS_DPP_FW(WDcfg.DppType)) {
    // Acquisition mode (LIST, OSCILLOSCOPE or MIXED)
    ret |= CAEN_DGTZ_SetDPPAcquisitionMode(
        handle[brd], (CAEN_DGTZ_DPP_AcqMode_t)WDcfg.AcquisitionMode,
        CAEN_DGTZ_DPP_SAVE_PARAM_EnergyAndTime);
    if ((WDcfg.DigitizerModel == 730) || (WDcfg.DigitizerModel == 725) ||
        (WDcfg.DigitizerModel == 751))
      CAEN_DGTZ_WriteRegister(handle[brd], 0x8004,
                              1 << 17);  // Enable Extra Word
    // some specific settings in the global CTRL register
    ret |= CAEN_DGTZ_WriteRegister(handle[brd], 0x8004,
                                   (1 << 2));  // set trgout bidir
    if (WDcfg.DppType == DPP_PSD_751)
      CAEN_DGTZ_WriteRegister(handle[brd], 0x8004,
                              1 << 26);  // Set new probe mode in x751 models

    if ((WDcfg.DppType == DPP_PHA_730) || (WDcfg.DppType == DPP_PSD_730) ||
        (WDcfg.DppType == DPP_nPHA_724))
      ret |=
          CAEN_DGTZ_WriteRegister(handle[brd], 0x8004, 1);  // Enable auto-flush

    if (WDcfg.DppType == DPP_PSD_720) {
      ret |= CAEN_DGTZ_WriteRegister(
          handle[brd], 0x8004,
          1 << 17);  // Enable baseline or Extended Time stamp
      ret |= RegisterSetBits(handle[brd], 0x1080, 7, 7, 1,
                             WDcfg);  // Enable ext time stamp
    }
  }
  ...
  if ((WDcfg.RecordLength > 30000) &&
      ((WDcfg.DppType == DPP_PHA_730) ||
       (WDcfg.DppType == DPP_PSD_730)))  // HACK: Bug Fix: the library doesn't
                                         // support length > 32K
  ...
  switch (WDcfg.StartMode) {
    ...
    case START_MODE_SLAVE:
      ...
      ret |=
          CAEN_DGTZ_ReadRegister(handle[brd], CAEN_DGTZ_ACQ_CONTROL_ADD, &d32);
      ret |= CAEN_DGTZ_WriteRegister(
          handle[brd], CAEN_DGTZ_ACQ_CONTROL_ADD,
          (d32 & 0xFFFFFFF0) |
  ...
  if (WDcfg.DigitizerModel != 5000) {
    if (WDcfg.TrginMode == TRGIN_MODE_DISABLED) {
      ret |= CAEN_DGTZ_WriteRegister(
          handle[brd], CAEN_DGTZ_TRIGGER_SRC_ENABLE_ADD,
          0x80000000);  // accept SW trg only (ext trig is disabled)
    } else if (WDcfg.TrginMode == TRGIN_MODE_GLOBAL_TRG) {
      ret |=
          CAEN_DGTZ_WriteRegister(handle[brd], CAEN_DGTZ_TRIGGER_SRC_ENABLE_ADD,
                                  0xC0000000);  // accept ext trg_in or SW trg
    } else if ((WDcfg.TrginMode == TRGIN_MODE_VETO) ||
               (WDcfg.TrginMode == TRGIN_MODE_GATE)) {
      ret |=
          CAEN_DGTZ_WriteRegister(handle[brd], CAEN_DGTZ_TRIGGER_SRC_ENABLE_ADD,
                                  0x80000000);  // accept SW trg only (ext trig
                                                // is used for the veto/gate)
      ret |= RegisterSetBits(
          handle[brd], 0x811C, 10, 11, 3,
          WDcfg);  // propagate ext-trg "as is" to channels (will be
                   // used as a validation for the self triggers)
      for (i = 0; i < 8; i++)
        ret |= CAEN_DGTZ_WriteRegister(handle[brd], 0x8180 + i * 4,
                                       0);  // not used
      if (WDcfg.DppType == DPP_PSD_730)
        ret |=
            RegisterSetBits(handle[brd], 0x8084, 4, 5, 1,
                            WDcfg);  // set individual trgin mode = from MB (not
                                     // used, masks are disabled)
      if (WDcfg.DppType == DPP_PHA_730)
        ret |=
            RegisterSetBits(handle[brd], 0x80A0, 4, 5, 1,
                            WDcfg);  // set individual trgin mode = from MB (not
                                     // used, masks are disabled)

    } else if (WDcfg.TrginMode ==
    ...
    if (WDcfg.TrginMode == TRGIN_MODE_DISABLED) {
      // ret |= RegisterSetBits(handle[brd], 0x8048, 2, 2, 0);   // disable
      // external trigger
    } else if (WDcfg.TrginMode == TRGIN_MODE_GLOBAL_TRG) {
      ret |= RegisterSetBits(handle[brd], 0x8048, 2, 2, 1,
                             WDcfg);  // enable external trigger
  ...
  if (WDcfg.DigitizerModel != 5000) {
    if (WDcfg.TrgoutMode == TRGOUT_MODE_DISABLED) {  // disabled (OR mask=0)
      ...
      ret |= RegisterSetBits(handle[brd], 0x811C, 16, 17, 0x1, WDcfg);
      ret |= RegisterSetBits(handle[brd], 0x811C, 18, 19, 0x0, WDcfg);
    } else if (WDcfg.TrgoutMode == TRGOUT_CLOCK) {  // propagate internal clock
      ret |= CAEN_DGTZ_WriteRegister(
          handle[brd], CAEN_DGTZ_FRONT_PANEL_IO_CTRL_ADD, 0x00050000);
      ...
      if (WDcfg.DppType ==
        ...
        ret |= RegisterSetBits(handle[brd], 0x8084, 20, 23, 7,
                               WDcfg);  // 1  => overth;
                                        // 2  => selftrg;
                                        // 3  => pu_trg;
        // 4  => pu_trg or selftrg;
      ...
      ret |= RegisterSetBits(handle[brd], 0x811C, 15, 15, 1, WDcfg);
      if (WDcfg.TrgoutMode == TRGOUT_MODE_SQR_1KHZ)
        ret |= RegisterSetBits(handle[brd], 0x8168, 0, 2, 1,
                               WDcfg);  // 1 KHz square wave
      if (WDcfg.TrgoutMode == TRGOUT_MODE_PLS_1KHZ)
        ret |= RegisterSetBits(handle[brd], 0x8168, 0, 2, 2,
                               WDcfg);  // 1 KHz pulses
      if (WDcfg.TrgoutMode == TRGOUT_MODE_SQR_10KHZ)
        ret |= RegisterSetBits(handle[brd], 0x8168, 0, 2, 3,
                               WDcfg);  // 10 KHz square wave
      if (WDcfg.TrgoutMode == TRGOUT_MODE_PLS_10KHZ)
        ret |= RegisterSetBits(handle[brd], 0x8168, 0, 2, 4,
                               WDcfg);  // 10 KHz pulses
    }
  } else {  // Hegagon
  ...
  if (IS_DPP_FW(WDcfg.DppType)) {
    ret |=
        CAEN_DGTZ_SetDPPEventAggregation(handle[brd], WDcfg.EventBuffering, 0);
    ret |= CAEN_DGTZ_SetMaxNumAggregatesBLT(handle[brd],
                                            1023);  // Number of buffers per BLT
  ...
  if (0) {  // manual settings (for debug)
    ...
    if (WDcfg.AcquisitionMode == ACQ_MODE_MIXED) {
      ...
      CAEN_DGTZ_WriteRegister(handle[brd], 0x8034,
                              1);  // Number of events per buffer
    } else {
      CAEN_DGTZ_WriteRegister(handle[brd], 0x800C,
                              0xA);  // Buffer organization (1024 buffers)
      CAEN_DGTZ_WriteRegister(handle[brd], 0x8034,
                              32);  // Number of events per buffer
    }
  }
  ...
  // ########################################################################################################
  // Hardware coincidence; HACK: this section has been tested only for few cases
  // and board models!!! Principle of operation: when the coincidence are
  // enabled in hardware (FPGA), each channel makes its own trigger
  // (self-trigger) which must be validated by a trigger validation signal
  // coming from the central FPGA in the mother board. There is a trigger mask
  // (one per channel) that defines the logic used to generate the trigger
  // validations on channel basis. This can be the OR, AND or Majority of the
  // self triggers coming from the participating channels. it is also possible
  // to include the external trigger (TRG-IN connector) and the LVDS I/Os.
  // NOTE1: in the x730, two channels (couple) share the same trigger mask and
  // trigger validation signal, so the coincidence takes place on couple basis;
  ...
  // validation signal to reject events intead of accept (anti-coincidence). The
  //        logic of the trigger mask does not change
  // NOTE3: besides the individual trigger validation signals, there is a common
  // signal (typically coming from TRG-IN) that can be used as a global inhibit
  //        or gate for the self triggers. This option is not managed here.

  // INDIVIDUAL_TRIGGER_MASK_REGISTER (address 0x8080 + i*4, i=channel/couple
  // number) [ 7: 0] = participating channel mask (refers to couples in the
  // x730): defines which self-triggers (i.e. from which channels) go into the
  // combination [ 9: 8] = combination logic: 0=OR, 1=AND, 2=MAJORITY [12:10] =
  // majority level [29] = enable LVDS(i) to be ORed in the trigger validation
  // [30] = enable external trigger to be ORed in the trigger validation

  if (((WDcfg.CoincMode > 0) || (WDcfg.TrginMode == TRGIN_MODE_VETO) ||
       (WDcfg.TrginMode == TRGIN_MODE_GATE)) &&
      IS_DPP_FW(WDcfg.DppType)) {
    char Msg[500];
    ...
    for (i = 0; i < MAX_NCH; i++) {
      if (WDcfg.EnableInput[brd][i] && (i < WDcfg.NumPhyCh)) {
        ...
        maskindex =
            ((WDcfg.DigitizerModel == 730) || (WDcfg.DigitizerModel == 725))
                ? i / 2
                : i;
        DppCtrl2 = (WDcfg.DppType == DPP_PHA_730) ? 0x10A0 : 0x1084;

        if (paired_channels) {
          ret |= RegisterSetBits(handle[brd], DppCtrl2 + (i << 8), 2, 2, 1,
                                 WDcfg);  // enable couple trgout
          ret |= RegisterSetBits(handle[brd], DppCtrl2 + (i << 8), 6, 6, 1,
                                 WDcfg);  // enable trg validation input
        }
        ...
        if (WDcfg.TrginMode == TRGIN_MODE_VETO) {  // TRGIN = VETO
          ...
          ret |= RegisterSetBits(
              handle[brd], 0x8080, 18, 19, 3,
              WDcfg);  // enable anti-coincindence mode (wait for
                       // trg validation to reject)
          if ((WDcfg.DppType == DPP_PHA_730) && (WDcfg.FWrev >= 4)) {
            ret |= CAEN_DGTZ_WriteRegister(
                handle[brd], 0x10D4 + (i << 8),
                vwreg);  // gate window width (0 = as long as input signal)
            ret |= RegisterSetBits(handle[brd], DppCtrl2 + (i << 8), 14, 15, 1,
                                   WDcfg);  // set veto source = TRGIN
          } else if (WDcfg.DppType == DPP_PSD_730) {
            ret |=
                CAEN_DGTZ_WriteRegister(handle[brd], 0x10D4 + (i << 8), vwreg);
            ret |= RegisterSetBits(handle[brd], DppCtrl2 + (i << 8), 18, 19, 1,
                                   WDcfg);  // set veto source = TRGIN
          }
          ret |=
              CAEN_DGTZ_WriteRegister(handle[brd], 0x8180 + maskindex * 4, 0);
        } else if (WDcfg.TrginMode ==
                   TRGIN_MODE_GATE) {  // TRGIN = GATE (= anti-veto)
          sprintf(Msg, "INFO: external TRGIN acts as a GATE\n");
          ret |= RegisterSetBits(handle[brd], 0x8080, 18, 19, coincmode,
                                 WDcfg);  // enable coincindence mode (wait
          ...
          if ((WDcfg.DppType == DPP_PHA_730) && (WDcfg.FWrev >= 4)) {
            ret |= CAEN_DGTZ_WriteRegister(
                ...
                vwreg);  // gate window width (0 = as long as input signal)
            ret |= RegisterSetBits(
                handle[brd], DppCtrl2 + (i << 8), 14, 15, 1,
                WDcfg);  // set veto source = TRGIN (antiveto = gate)
          } else if (WDcfg.DppType == DPP_PSD_730) {
            ret |=
                CAEN_DGTZ_WriteRegister(handle[brd], 0x10D4 + (i << 8), vwreg);
          }
          ...
          if ((WDcfg.DigitizerModel == 724) || (WDcfg.DigitizerModel == 780) ||
              (WDcfg.DigitizerModel == 781))
            ret |= CAEN_DGTZ_WriteRegister(handle[brd], 0x1084 + (i << 8),
                                           WDcfg.CoincWindow / 10);
          else if (WDcfg.DppType == DPP_PHA_730)
            ret |= CAEN_DGTZ_WriteRegister(handle[brd], 0x1084 + (i << 8),
                                           WDcfg.CoincWindow / stu);
          else
            ret |= CAEN_DGTZ_WriteRegister(handle[brd], 0x1070 + (i << 8),
                                           WDcfg.CoincWindow / stu);
          ...
          switch (WDcfg.CoincMode) {
            case COINC_MAJORITY:  // acquire when the num of fired channels >=
              ...
              if (paired_channels) {
                sprintf(
                    Msg,
                    "INFO: Coincidence with Majority (fired couples >= %d)\n",
                    WDcfg.MajorityLevel);
                ret |= RegisterSetBits(handle[brd], DppCtrl2 + (i << 8), 0, 1,
                                       3, WDcfg);  // couple trgout = OR
                ret |= RegisterSetBits(
                    handle[brd], DppCtrl2 + (i << 8), 4, 5, 1,
                    WDcfg);  // trigger validation comes from the other couples
              } else {
                sprintf(
                    Msg,
                    "INFO: Coincidence with Majority (fired channels >= %d)\n",
                    WDcfg.MajorityLevel);
              }
              ret |= RegisterSetBits(handle[brd], 0x8080, 18, 19, coincmode,
                                     WDcfg);  // enable coincindence mode (wait
              ...
              ret |= CAEN_DGTZ_WriteRegister(
                  handle[brd], 0x8180 + maskindex * 4,
                  0x200 | ChTrgMask | ((WDcfg.MajorityLevel - 1) << 10));
              break;
            ...
            case COINC_MINORITY:  // acquire when the num of fired channels <
              ...
              if (paired_channels) {
                sprintf(
                    Msg,
                    "INFO: Coincidence with Minority (fired couples < %d)\n",
                    WDcfg.MajorityLevel);
                ret |= RegisterSetBits(handle[brd], DppCtrl2 + (i << 8), 0, 1,
                                       3, WDcfg);  // couple trgout = OR
                ret |= RegisterSetBits(
              ...
              ret |= RegisterSetBits(
                  handle[brd], 0x8080, 18, 19, 3,
                  WDcfg);  // enable anti-coincindence mode (wait
                           // for trg validation to reject)
              ret |= CAEN_DGTZ_WriteRegister(
                  handle[brd], 0x8180 + maskindex * 4,
                  0x200 | ChTrgMask | ((WDcfg.MajorityLevel - 1) << 10));
            ...
            case COINC_PAIRED_AND:  // both channels of a couple acquire when
              ...
              if (paired_channels) {
                ret |=
                    CAEN_DGTZ_WriteRegister(handle[brd], 0x8180 + maskindex * 4,
                                            0);  // signals from mother-board
                                                 // (inter-couple) are not used
                ret |= RegisterSetBits(
                    handle[brd], DppCtrl2 + (i << 8), 4, 5, 2,
                    WDcfg);  // trigger validation comes from the
                             // other channel in the couple
                ret |= RegisterSetBits(handle[brd], DppCtrl2 + (i << 8), 0, 1,
                                       0, WDcfg);  // couple trgout = AND
                ret |= CAEN_DGTZ_WriteRegister(
                    handle[brd], 0x106C + (i << 8),
                    2);  // extra-time window shorter incase of paired channels
              } else {
                if ((WDcfg.DigitizerModel == 780) ||
                    (WDcfg.DigitizerModel == 790))
                  ret |= CAEN_DGTZ_WriteRegister(handle[brd],
              ...
              WDcfg.TrgHoldOff =
                  myMax((2 * WDcfg.CoincWindow), WDcfg.TrgHoldOff);
              break;
            ...
            case COINC_PAIRED_NAND:  // one channel of a couple acquires when
                                     ...
                                     // coincidence)
              sprintf(Msg,
                      "INFO: Anti-Coincidence in Paired Mode: Not(CH[even] AND "
                      "CH[odd])\n");
              ret |= RegisterSetBits(
                  handle[brd], 0x8080, 18, 19, 3,
                  WDcfg);  // enable anti-coincindence mode (wait
              ...
              if (paired_channels) {
                ...
                ret |= RegisterSetBits(
                    ...
                    WDcfg);  // trigger validation comes from the
                             // other channel in the couple
                ret |= RegisterSetBits(
                    handle[brd], DppCtrl2 + (i << 8), 0, 1, 3,
                    WDcfg);  // couple trgout = OR (NOTE: doesn't
                             // reflect the acq. trigger!!!)
                ret |= CAEN_DGTZ_WriteRegister(
                    handle[brd], 0x106C + (i << 8),
                    0);  // don't need extra-time for the coincidence window
              } else {
                ret |=
                    CAEN_DGTZ_WriteRegister(handle[brd], 0x8180 + maskindex * 4,
                                            0x100 | (3 << (i / 2)));
            ...
            case COINC_PAIRED_OR:  // both channels of a couple acquire when one
              ...
              if (WDcfg.DppType == DPP_PHA_724) {
                ...
                sprintf(Msg,
                        ...
                        "CH[odd]\n");
                if (WDcfg.DppType != DPP_PHA_724)
                  ret |= RegisterSetBits(handle[brd], 0x1080 + (i << 8), 24, 24,
                                         1, WDcfg);  // disable self trg
                if (paired_channels) {
                  ret |= CAEN_DGTZ_WriteRegister(
                      handle[brd], 0x8180 + maskindex * 4,
                      0);  // signals from mother-board (inter-couple) are not
                           // used
                  ret |= RegisterSetBits(
                      handle[brd], DppCtrl2 + (i << 8), 4, 5, 3,
                      WDcfg);  // trigger propagation between the
                  ...
                  ret |= CAEN_DGTZ_WriteRegister(
                      handle[brd], 0x8180 + maskindex * 4, 3 << (i / 2));
                  /*if ((i & 1) == 0)
                          ret |= CAEN_DGTZ_WriteRegister(handle[brd], 0x8180 +
                  maskindex*4, 0x100 | 2 << i);  // even else ret |=
                  CAEN_DGTZ_WriteRegister(handle[brd], 0x8180 + maskindex*4,
                  0x100 | 1 << (i-1));  // odd*/
                }
              }
            ...
            case COINC_CH0_NAND_ANY:
              if (WDcfg.CoincMode == COINC_CH0_AND_ANY) {
                ret |= RegisterSetBits(
                    handle[brd], 0x8080, 18, 19, coincmode,
                    WDcfg);  // enable coincindence mode (wait for trg
              ...
              if (paired_channels) {
                if (i == 0) {
                  ret |= RegisterSetBits(
                      handle[brd], DppCtrl2, 0, 1, 1,
                      WDcfg);  // couple trgout = ch0 only (for couple 0)
                  ret |= RegisterSetBits(
                      handle[brd], DppCtrl2, 4, 5, 1,
                      WDcfg);  // trigger validation comes from other couples
                  ret |= CAEN_DGTZ_WriteRegister(handle[brd], 0x8180,
                                                 0xFE & ChTrgMask);
                } else if (i == 1) {
                  ret |= RegisterSetBits(
                      handle[brd], 0x1080 + (i << 8), 24, 24, 1,
                      WDcfg);  // disable self trg (ch1 cannot be
                  ...
                  /*ret |= RegisterSetBits(handle[brd], DppCtrl2 + (i<<8), 0, 1,
                  3);  // couple trgout = OR ret |= RegisterSetBits(handle[brd],
                  DppCtrl2 + (i<<8), 4, 5, 1);  // trigger validation comes from
                  other couples ret |= CAEN_DGTZ_WriteRegister(handle[brd],
                  0x8180 + maskindex*4, 0x01);*/
                  if (WDcfg.CoincMode == COINC_CH0_AND_ANY) {
                    ret |= RegisterSetBits(handle[brd], DppCtrl2 + (i << 8), 0,
                                           1, 3, WDcfg);  // couple trgout = OR
                    ret |= RegisterSetBits(
                        handle[brd], DppCtrl2 + (i << 8), 4, 5, 1,
                        WDcfg);  // trigger validation comes from other couples
                    ret |= CAEN_DGTZ_WriteRegister(
                        handle[brd], 0x8180 + maskindex * 4, 0x01);
                  } else {
                    ret |= RegisterSetBits(
                        handle[brd], 0x1080 + (i << 8), 18, 19, 0,
                        WDcfg);  // other channels acquires in singles
                    ret |= RegisterSetBits(handle[brd], DppCtrl2 + (i << 8), 0,
                                           1, 3, WDcfg);  // couple trgout = OR
                    ret |= CAEN_DGTZ_WriteRegister(
                        handle[brd], 0x8180 + maskindex * 4, 0x00);
                  }
                }
              } else {
                if (i == 0)
            ...
            case COINC_AND_ALL:  // coincidence between all the enabled channels
              ...
              ret |= RegisterSetBits(handle[brd], 0x8080, 18, 19, coincmode,
                                     WDcfg);  // enable coincindence mode (wait
                                              // for trg validation to acquire)
              ret |= CAEN_DGTZ_WriteRegister(
                  handle[brd], 0x8180 + maskindex * 4, 0x100 | ChTrgMask);
              if (paired_channels) {
                ret |= RegisterSetBits(
                    handle[brd], DppCtrl2 + (i << 8), 4, 5, 1,
                    WDcfg);  // trigger validation comes from other couples
                if ((i & 1) == 0) {
                  if (WDcfg.EnableInput[brd][i + 1])
                    ret |= RegisterSetBits(handle[brd], DppCtrl2 + (i << 8), 0,
                                           1, 0, WDcfg);  // couple trgout = AND
                  else
                    ret |=
                        RegisterSetBits(handle[brd], DppCtrl2 + (i << 8), 0, 1,
                                        1, WDcfg);  // couple trgout = even only
                } else {
                  if (WDcfg.EnableInput[brd][i - 1])
                    ret |= RegisterSetBits(handle[brd], DppCtrl2 + (i << 8), 0,
                                           1, 0, WDcfg);  // couple trgout = AND
                    ...
                    ret |=
                        RegisterSetBits(handle[brd], DppCtrl2 + (i << 8), 0, 1,
                                        2, WDcfg);  // couple trgout = odd only
                }
              }
              WDcfg.TrgHoldOff =
                  myMax((2 * WDcfg.CoincWindow), WDcfg.TrgHoldOff);
            ...
            case COINC_OR_ALL:  // any self trigger makes all channels to
              ...
              if (WDcfg.DppType == DPP_PHA_724) {
                ...
                ret |= RegisterSetBits(handle[brd], 0x1080 + (i << 8), 24, 24,
                                       1, WDcfg);  // disable self trg
                ret |= CAEN_DGTZ_WriteRegister(
                    handle[brd], 0x8180 + maskindex * 4, ChTrgMask);
                if (paired_channels) {
                  ret |= RegisterSetBits(
                      handle[brd], DppCtrl2 + (i << 8), 4, 5, 1,
                      WDcfg);  // trigger propagation comes from other couples
                  ret |= RegisterSetBits(handle[brd], DppCtrl2 + (i << 8), 0, 1,
                                         3, WDcfg);  // couple trgout = OR
                }
              }
  ...
  for (i = 0; i < MAX_NCH; i++) {
    if ((WDcfg.EnableInput[brd][i]) && (i < WDcfg.NumPhyCh)) {
      // DC offset
      // Old setting: WDcfg.DCoffset goes from -50% to 50%; pulse polarity is
      // not taken into account New setting: WDcfg.BaselineDCoffset goes from 0%
      // to 100%, considering pulse polarity
      if (WDcfg.BaselineDCoffset[brd][i] >= 0) {
        ...
        if (WDcfg.PulsePolarity[brd][i] == 1)  // negative
          ...
          offs = (uint16_t)(((100 - WDcfg.BaselineDCoffset[brd][i]) * 65535) /
                            100);
        ret |= CAEN_DGTZ_SetChannelDCOffset(handle[brd], i, offs);
      } else {
        ret |= CAEN_DGTZ_SetChannelDCOffset(handle[brd], i,
                                            WDcfg.DCoffset[brd][i]);
      }
      ...
      if (WDcfg.DppType == DPP_PHA_730) {
        ...
        WDcfg.EnergyDiv[brd][i] =
            (1 << 14) /
            WDcfg.EHnbin;  // HACK: for the moment, the CAENDigitizer library
                           // divides the Energy by two to express it as a 14
                           // bit number

        // ret |= CAEN_DGTZ_WriteRegister(handle[brd], 0x1034 + (i<<8), 1);
        // // nevbuf
        ret |= CAEN_DGTZ_WriteRegister(handle[brd], 0x1038 + (i << 8),
                                       WDcfg.PreTrigger / 4);  // pretrg
        ...
        ret |= CAEN_DGTZ_WriteRegister(handle[brd], 0x1064 + (i << 8),
                                       ftd);  // ftd
        ret |=
            CAEN_DGTZ_WriteRegister(handle[brd], 0x1068 + (i << 8), MM);  // MM
        ret |= CAEN_DGTZ_WriteRegister(handle[brd], 0x106C + (i << 8),
                                       thr);  // thr
        ret |=
            CAEN_DGTZ_WriteRegister(handle[brd], 0x1070 + (i << 8), 0);  // tww
        ret |= CAEN_DGTZ_WriteRegister(handle[brd], 0x1074 + (i << 8),
                                       trgho);  // trgho
        ret |=
            CAEN_DGTZ_WriteRegister(handle[brd], 0x1078 + (i << 8), 8);  // pkho
        ret |=
            CAEN_DGTZ_WriteRegister(handle[brd], 0x107C + (i << 8), 4);  // blho
        // ret |= CAEN_DGTZ_WriteRegister(handle[brd], 0x1084 + (i<<8), 4);
        // // tvaw

        ret |= RegisterSetBits(handle[brd], 0x1080 + (i << 8), 0, 5, shf,
                               WDcfg);  // shf
        ...
        ret |= RegisterSetBits(handle[brd], 0x1080 + (i << 8), 16, 16,
        ...
        if (WDcfg.FWrev >= 4) {
          ret |= CAEN_DGTZ_WriteRegister(
              handle[brd], 0x10D0 + (i << 8),
              WDcfg.ZeroVoltLevel[brd]
                                 [i]);  // input baseline level in ADC counts
          if (WDcfg.DiscrMode[brd][i] ==
              DISCR_MODE_DISABLED) {  // Disable self trigger
            ret |= RegisterSetBits(handle[brd], 0x1080 + (i << 8), 24, 24, 1,
                                   WDcfg);
          } else if (WDcfg.DiscrMode[brd][i] ==
                     DISCR_MODE_CFD_PHA) {  // CFD_PHA
            ret |=
                RegisterSetBits(handle[brd], 0x1080 + (i << 8), 6, 6, 0, WDcfg);
            ret |= RegisterSetBits(handle[brd], 0x1080 + (i << 8), 17, 17, 1,
                                   WDcfg);
            ret |= RegisterSetBits(handle[brd], 0x10A0 + (i << 8), 12, 13,
                                   WDcfg.CFDfraction[brd][i], WDcfg);
          } else if (WDcfg.DiscrMode[brd][i] ==
                     DISCR_MODE_LED_PHA) {  // LED_PHA
            ret |=
                RegisterSetBits(handle[brd], 0x1080 + (i << 8), 6, 6, 1, WDcfg);
            ret |= RegisterSetBits(handle[brd], 0x1080 + (i << 8), 17, 17, 1,
                                   WDcfg);
          }
        } else if (WDcfg.DiscrMode[brd][i] ==
                   DISCR_MODE_DISABLED) {  // Disable self trigger
          ret |=
              RegisterSetBits(handle[brd], 0x1080 + (i << 8), 24, 24, 1, WDcfg);
        }

        ret |= CAEN_DGTZ_WriteRegister(
            handle[brd], 0x1028 + (i << 8),
            WDcfg.InputDynamicRange[brd][i] & 1);  // 0=2Vpp, 1=0.5Vpp
        ...
        ret |= RegisterSetBits(handle[brd], 0x1080 + (i << 8), 14, 14,
                               WDcfg.EnableIPE[brd][i] & 1,
                               WDcfg);  // enable test pulse
        if (WDcfg.FWrev <= 4) {
          uint32_t IPEperiod =
              WDcfg.IPErandom[brd][i]
                  ? 0xFFE0
                  : (uint32_t)(10000000 / WDcfg.IPEfrequency[brd][i]);
          ret |= CAEN_DGTZ_WriteRegister(
              handle[brd], 0x1044 + (i << 8),
              (WDcfg.IPErandom[brd][i] << 16) | (IPEperiod & 0xFFFF));
          // ret |= CAEN_DGTZ_WriteRegister(handle[brd], 0x1048 + (i<<8),
          // (uint32_t)(65536*exp(-1/(WDcfg.IPEdecay[brd][i]*125))) |
          // (WDcfg.IPErisetime[brd][i]<<16) | (WDcfg.IPEaddnoise[brd][i]<<19));
          ret |=
              CAEN_DGTZ_WriteRegister(handle[brd], 0x104C + (i << 8),
                                      WDcfg.IPEamplitude[brd][i]);  // amplitude
        } else {
          ...
          uint32_t reg;
          reg = ampl | (decay << 4) | (rise << 12) | (rand << 14) |
                (noise << 15) | (freq << 16);
        ...
        ret |= CAEN_DGTZ_SetDPPPreTriggerSize(handle[brd], i,
                                              WDcfg.PreTrigger);  // individual
        ...
        // ret |= CAEN_DGTZ_WriteRegister(handle[brd], 0x1038 + (i<<8),
        // (WDcfg.PreTrigger));	// pretrg (in samples)
        if (WDcfg.TTFsmoothing[brd][i] == 0) WDcfg.TTFsmoothing[brd][i] = 1;
        ret |= CAEN_DGTZ_WriteRegister(
            handle[brd], 0x1054 + (i << 8),
            (uint32_t)(WDcfg.TTFsmoothing[brd][i]));  // a
        ret |= CAEN_DGTZ_WriteRegister(handle[brd], 0x1058 + (i << 8),
                                       ttf_delay);  // b
        ret |= CAEN_DGTZ_WriteRegister(handle[brd], 0x105C + (i << 8),
                                       traprise);  // k
        ret |= CAEN_DGTZ_WriteRegister(
            handle[brd], 0x1060 + (i << 8),
            (uint32_t)(WDcfg.TrapFlatTop[brd][i] / tu));  // m
        ret |= CAEN_DGTZ_WriteRegister(
            handle[brd], 0x1064 + (i << 8),
            (uint32_t)(WDcfg.PeakingTime[brd][i] / tu));  // ftd
        ret |=
            CAEN_DGTZ_WriteRegister(handle[brd], 0x1068 + (i << 8), tau);  // MM
        ret |= CAEN_DGTZ_WriteRegister(
            handle[brd], 0x106C + (i << 8),
            (uint32_t)(WDcfg.TrgThreshold[brd][i]));  // thr
        ret |=
            CAEN_DGTZ_WriteRegister(handle[brd], 0x1070 + (i << 8), 0);  // tww
        if (WDcfg.DppType == DPP_nPHA_724)
          ret |= CAEN_DGTZ_WriteRegister(
              handle[brd], 0x1074 + (i << 8),
              (uint32_t)((float)WDcfg.TrgHoldOff / tu + 0.5));  // trgho
        else
          ret |= CAEN_DGTZ_WriteRegister(
              handle[brd], 0x1074 + (i << 8),
              (uint32_t)((float)WDcfg.TrgHoldOff / (8 * tu) + 0.5));  // trgho
        ret |= CAEN_DGTZ_WriteRegister(
            handle[brd], 0x1078 + (i << 8),
            (uint32_t)(WDcfg.PeakHoldOff[brd][i] / (tu * 8)));  // pkho
        ret |= CAEN_DGTZ_WriteRegister(handle[brd], 0x107C + (i << 8),
                                       1000);  // blho
        // ret |= CAEN_DGTZ_WriteRegister(handle[brd], 0x1084 + (i<<8), 4); //
        // tvaw
        ...
        // -----------------------------------
        if ((WDcfg.DigitizerModel == 780) || (WDcfg.DigitizerModel == 781)) {
          int drange;
          ...
          ret |= CAEN_DGTZ_WriteRegister(handle[brd], 0x10B4 + (i << 8),
                                         drange & 0xF);
        } else if (WDcfg.DigitizerModel == 5000) {
          int drange;
          if (0) {
            drange = WDcfg.InputDynamicRange[brd][i];  // direct drive gain
                                                       // register (for debug)
          } else {
            if (WDcfg.EnergyCoarseGain[brd][i] == 0.25)
              drange = 0x43;  // 9000 mVpp - Att = 4; Gain = 1
            else if (WDcfg.EnergyCoarseGain[brd][i] == 0.5)
              drange = 0x45;  // 4500 mVpp - Att = 4; Gain = 2
            else if (WDcfg.EnergyCoarseGain[brd][i] == 1)
              drange = 0x42;  // 2250 mVpp - Att = 1; Gain = 1
            else if (WDcfg.EnergyCoarseGain[brd][i] == 2)
              drange = 0x44;  // 1125 mVpp - Att = 1; Gain = 2
            else if (WDcfg.EnergyCoarseGain[brd][i] == 4)
              drange = 0x48;  //  562 mVpp - Att = 1; Gain = 4
            else if (WDcfg.EnergyCoarseGain[brd][i] == 8)
              drange = 0x50;  //  281 mVpp - Att = 1; Gain = 8
            else if (WDcfg.EnergyCoarseGain[brd][i] == 16)
              drange = 0x60;  //  140 mVpp - Att = 1; Gain = 16
            else if (WDcfg.EnergyCoarseGain[brd][i] == 32)
              drange = 0x90;  //   70 mVpp - Att = 1; Gain = 32
            else if (WDcfg.EnergyCoarseGain[brd][i] == 64)
              drange = 0xA0;  //   35 mVpp - Att = 1; Gain = 64
            else if (WDcfg.EnergyCoarseGain[brd][i] == 128)
              drange = 0x110;  //  17 mVpp - Att = 1; Gain = 128
            else if (WDcfg.EnergyCoarseGain[brd][i] == 256)
              drange = 0x120;  //   8 mVpp - Att = 1; Gain = 256
            else if (WDcfg.EnergyCoarseGain[brd][i] == 500)
              drange = 0x13E;  //   4 mVpp - Att = 1; Gain = 500
            else
              drange = 0x42;  // bad setting: use gain = 1
          ...
          ret |= RegisterSetBits(
              handle[brd], 0x10BC + (i << 8), 0, 1, WDcfg.ACcoupling[brd][i],
              WDcfg);  // 0=disabled, 1=1.8us, 2=4.7us, 3=10us
          ret |= CAEN_DGTZ_WriteRegister(
              handle[brd], 0x10B0 + (i << 8),
              WDcfg.ACPZcomp[brd][i]);  // DAC for AC Pole Zero compensation

          // memory test mode (replace adc data with known patterns)
          if (0) ret |= RegisterSetBits(handle[brd], 0x8000, 3, 3, 1, WDcfg);
        }

        if (WDcfg.DigitizerModel == 5000)
          Tg = (tau * traprise) /
               WDcfg.EnergyFineGain[brd][i];  // coarse gain is hardware
        else
          Tg = (tau * traprise) /
               (WDcfg.EnergyCoarseGain[brd][i] * WDcfg.EnergyFineGain[brd][i]);
        // set trapezoid gain and nroma factor
        ...
        // the board provides energy with 15 bits (32K channels); for the
        // x724/x780 the CAENDigitizer library divide by 2 and gives the energy
        // on 14 bits calculate rebinning factor to go into WDcfg.EHnbin
        // channels.
        if (WDcfg.DigitizerModel == 5000)
        ...
        if (WDcfg.DiscrMode[brd][i] == DISCR_MODE_DISABLED) {
          if (WDcfg.DigitizerModel == 5000)
            ret |= RegisterSetBits(handle[brd], 0x1048 + (i << 8), 0, 0, 0,
                                   WDcfg);  // Disable self trigger
          else
            ret |= RegisterSetBits(handle[brd], 0x1080 + (i << 8), 24, 24, 1,
                                   WDcfg);  // Disable self trigger
        }
        ...
        if (WDcfg.DppType == DPP_nPHA_724) {
          ...
          ret |= CAEN_DGTZ_WriteRegister(handle[brd], 0x10C8 + (i << 8),
                                         (uint32_t)WDcfg.EnergyLCut[brd][i]);
          ret |= CAEN_DGTZ_WriteRegister(handle[brd], 0x10CC + (i << 8),
                                         (uint32_t)WDcfg.EnergyUCut[brd][i]);
          ret |= RegisterSetBits(handle[brd], 0x1080 + (i << 8), 28, 28,
                                 WDcfg.EnableEnergySkim, WDcfg);
        }

        // +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        // PSD_720/751/730/725 + CI_720
        // +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      } else if ((WDcfg.DppType == DPP_PSD_720) ||
                 (WDcfg.DppType == DPP_PSD_730) ||
                 (WDcfg.DppType == DPP_PSD_751) || (WDcfg.DppType == DPP_CI)) {
        if (WDcfg.DppType == DPP_CI)
          ret |= CAEN_DGTZ_SetDPPPreTriggerSize(
              handle[brd], -1, WDcfg.PreTrigger);  // common to all channels
        else
          ret |= CAEN_DGTZ_SetDPPPreTriggerSize(
              handle[brd], i, WDcfg.PreTrigger);  // individual

        // *****************************************
        // Energy Gain (ChargeSensitivity)
        // *****************************************
        // The PSD and CI firmware uses the parameter ChargeSensitivity to set
        // the conversion gain from charge to 1 LSB (channel) and to define the
        // full scale range of the energy spectrum. However, for many users it
        // is more familiar to use the "CoarseGain" for this purpose. If the
        // ChargeSensitivity is not set in the config file (default=-1), then
        // the CoarseGain is used to calculate it automatically according to the
        // convertion tables below. NOTE: in some boards, the ChargeSensitivity
        // goes in steps of 4 instead of 2. In this cases, the intermediate
        // CoarseGains are obtained by a mathematical division by 2 applied in
        // the software (Energy Divider). The boards (all of them) provide the
        // energy (=charge) with 16 bits (64k channels); the energy value is
        // divided by 2^N to fit into the wanted number of channels (parameter
        // EHnbin). Allowed values for the CoarseGain are 1/16, 1/8, 1/4, 1/2,
        // 1, 2, 4, 8, 16, 32 (not all boards support all values)

        // Analog Sensitivity in fC/ch (charge represented with 16 bit => 64K
        // channels)
        //				   CoarseGain =	1/16	1/8
        //1/4		1/2		1		2		4		8
        //16		32
        // ------------------------------------------------------------------------------------------------------------
        // x730 - 2 Vpp					2560	1280	640
        // 320		160		80		40		20		10
        // 5
        // x730 - 0.5 Vpp				640		320		160
        // 80		40		20		10		5		2.5
        // 1.25 x725 - 2 Vpp					2560
        // 1280	640		320		160		80		40
        // 20		10		---
        // x725 - 0.5 Vpp 				640		320		160
        // 80		40		20		10		5		2.5
        // ---
        // x751 - 1 Vpp 				---		640		320
        // 160		80		40		20		---		---
        ...
        //16		32
        // ------------------------------------------------------------------------------------------------------------
        // x730 - 2 Vpp					4 - 2	4 - 1	3 - 2	3
        // - 1	2 - 2	2 - 1	1 - 2	1 - 1	0 - 2	0 - 1 x730 - 0.5 Vpp
        // 4 - 2	4 - 1	3 - 2	3 - 1	2 - 2	2 - 1	1 - 2
        // 1 - 1	0 - 2	0 - 1
        // x725 - 2 Vpp					4 - 1	3 - 2	3 - 1	2
        // - 2	2 - 1	1 - 2	1 - 1	0 - 2	0 - 1	--- x725 - 0.5 Vpp
        // 4 - 1	3 - 2	3 - 1	2 - 2	2 - 1	1 - 2	1 - 1
        // 0 - 2	0 - 1	---
        // x751 - 1 Vpp 				---		5 - 1	4 - 1	3 -
        // 1	2 - 1	1 - 1	0 - 1	---		---		--- x720
        // - 2 Vpp	  				---		---		2 - 1
        // 1 - 2	1 - 1	0 - 2	0 - 1	---		---
        // ---
        ...
        //16		32
        // ------------------------------------------------------------------------------------------------------------
        // x730 - 2 Vpp					800		400		200
        // 100		50		25		12		6		3
        // 1.5
        // x730 - 0.5 Vpp				200		100		50
        // 25		12		6		3		1.5		0.8
        // 0.4
        // x725 - 2 Vpp					800		400		200
        // 100		50		25		12		6		3
        // ---
        // x725 - 0.5 Vpp 				200		100		50
        // 25		12		6		3		1.5		0.8
        // ---
        // x751 - 1 Vpp 				---		200		100
        // 50		25		12		6		---		---
        ...
        if (WDcfg.ChargeSensitivity[brd][i] < 0) {
          ...
          // the board provides energy with 16 bits (64K channels); calculate
          // rebinning factor to go into WDcfg.EHnbin channels.
          rebin = (1 << 16) /
                  WDcfg.EHnbin;  // Max value for EHnbin = 32K => rebin >= 2

          // convert the coarse gain option (0 to 9) into the relevant setting
          // for the charge sensitivity and for the energy multiplier that takes
          // into account the rebinning and the gaps (2 by 2) in the sensitivity
          // (where present)
          if (WDcfg.DigitizerModel == 730) {
            switch (gc) {
              case 0:
        ...
        // Trigger Threshold
        ret |= CAEN_DGTZ_WriteRegister(handle[brd], 0x1060 + (i << 8),
                                       WDcfg.TrgThreshold[brd][i]);

        // Trigger Holdoff
        ret |= CAEN_DGTZ_WriteRegister(handle[brd], 0x1074 + (i << 8),
                                       WDcfg.TrgHoldOff / stu);

        // Gate Settings
        ret |= CAEN_DGTZ_WriteRegister(handle[brd], 0x1058 + (i << 8),
                                       WDcfg.GateWidth[brd][i] / WDcfg.Tsampl);
        ret |= CAEN_DGTZ_WriteRegister(
            handle[brd], 0x1054 + (i << 8),
            WDcfg.ShortGateWidth[brd][i] / WDcfg.Tsampl);
        ret |= CAEN_DGTZ_WriteRegister(handle[brd], 0x105C + (i << 8),
                                       WDcfg.PreGate[brd][i] / WDcfg.Tsampl);
        ...
        if (WDcfg.DiscrMode[brd][i] == DISCR_MODE_DISABLED) {
          ret |= RegisterSetBits(handle[brd], 0x1080 + (i << 8), 24, 24, 1,
                   ...
                   ((WDcfg.DppType == DPP_PSD_751) && (WDcfg.FWrev >= 8))) {
          ret |= RegisterSetBits(handle[brd], 0x103C + (i << 8), 0, 7,
                                 WDcfg.CFDdelay[brd][i] / WDcfg.Tsampl, WDcfg);
          ret |= RegisterSetBits(handle[brd], 0x103C + (i << 8), 8, 9,
                                 WDcfg.CFDfraction[brd][i], WDcfg);
          ret |= RegisterSetBits(handle[brd], 0x103C + (i << 8), 10, 11,
                                 WDcfg.CFDinterp[brd][i], WDcfg);
        ...
        if (WDcfg.EnablePedestal[brd][i])
          ret |= RegisterSetBits(handle[brd], 0x1080 + (i << 8), 4, 4, 1,
                                 WDcfg);  // enable pedestal

        // Charge Sensitivity
        if ((WDcfg.DigitizerModel == 730) || (WDcfg.DigitizerModel == 725))
          ret |= RegisterSetBits(handle[brd], 0x1080 + (i << 8), 0, 2,
                                 WDcfg.ChargeSensitivity[brd][i], WDcfg);
        else
          ret |= RegisterSetBits(handle[brd], 0x1080 + (i << 8), 0, 1,
                                 WDcfg.ChargeSensitivity[brd][i], WDcfg);

        // Pulse Polarity
        ret |= RegisterSetBits(handle[brd], 0x1080 + (i << 8), 16, 16,
                               WDcfg.PulsePolarity[brd][i], WDcfg);

        // Disable Trigger hysteresis
        if (0)
          ret |=
              RegisterSetBits(handle[brd], 0x1080 + (i << 8), 30, 30, 1, WDcfg);

        // Disable Negative trigger detection
        if (0)
          ret |=
              RegisterSetBits(handle[brd], 0x1080 + (i << 8), 31, 31, 1, WDcfg);

        // Force Baseline calculation to restart at the end of the long gate,
        // regardless the signal over/under thr
        if (0)
          ret |=
              RegisterSetBits(handle[brd], 0x1080 + (i << 8), 15, 15, 1, WDcfg);

        // Enable Retriggering while busy (used to replicate selftrg onto
        // TRGOUT)
        if (0)
          ret |=
              RegisterSetBits(handle[brd], 0x1080 + (i << 8), 5, 5, 1, WDcfg);
        ...
        if (0) {
          ret |=
              RegisterSetBits(handle[brd], 0x1080 + (i << 8), 3, 3, 1, WDcfg);
        ...
        // Qthr) The ZS is only available for 730/725 starting from FW-rev
        // 130.09
        if ((WDcfg.ChargeLLD[brd][i] > 0) && (WDcfg.DppType == DPP_PSD_730) &&
            (WDcfg.FWrev > 9)) {
        ...
        if ((WDcfg.DigitizerModel == 730) || (WDcfg.DigitizerModel == 725))
          ret |= RegisterSetBits(handle[brd], 0x1084 + (i << 8), 11, 11,
                                 WDcfg.SmoothedForCharge[brd][i], WDcfg);

        // Internal Pulse Emulator
        if (WDcfg.EnableIPE[brd][i]) {
          ret |=
              RegisterSetBits(handle[brd], 0x1080 + (i << 8), 8, 8, 1, WDcfg);
        ...
        // Downgrade emulation
        if ((WDcfg.DigitizerModel == 730) || (WDcfg.DigitizerModel == 725)) {
          int dgrade = 0;  // 0=14bit, 1=13bit, 2=12bit, 3=10bit. NOTE: the
                           // sample is still represented over 14 bits; the FPGA
                           // forces 1, 2, or 4 LSBs to zero
          int dsamp =
              0;  // 0=500MSps 1=250MSps NOTE: this is not a real downsampling.
                  // The FPGA sets odd samples = even samples (s0, s1, s2, s3...
                  // become s0, s0, s2, s2...)
        ...
        if (WDcfg.PSDcutThr[brd][i] != 0) {
          ret |= CAEN_DGTZ_WriteRegister(
              handle[brd], 0x1078 + (i << 8),
              (uint32_t)(1024 * WDcfg.PSDcutThr[brd][i]));  // Set threshold
          if (WDcfg.PSDcutType[brd][i] == 0)
            ret |= RegisterSetBits(handle[brd], 0x1080 + (i << 8), 27, 28, 2,
                                   WDcfg);  // keep neutrons only
          else
            ret |= RegisterSetBits(handle[brd], 0x1080 + (i << 8), 27, 28, 1,
                                   WDcfg);  // keep gammas only
        }

        // Pile-Up settings
        // PUR Mode: 0=disabled, 1=reject, 2=close current gate (truncated) and
        // open a new one (x751 only)
        ret |= CAEN_DGTZ_WriteRegister(handle[brd], 0x107c + (i << 8),
                                       WDcfg.PurGap[brd][i]);
        if (WDcfg.PileUpMode[brd][i] == 1) {
          ret |=
              RegisterSetBits(handle[brd], 0x1080 + (i << 8), 26, 26, 1, WDcfg);
          ret |= RegisterSetBits(
              handle[brd], 0x1080 + (i << 8), 7, 7, 1,
              WDcfg);  // Enable the use of Pile-ups to make triggers (for counting
                       // ICR or generating trgout pulses)
        } else if ((WDcfg.PileUpMode[brd][i] == 2) &&
                   (WDcfg.DigitizerModel == 751)) {
          ret |=
              RegisterSetBits(handle[brd], 0x1080 + (i << 8), 25, 25, 1, WDcfg);
        } else {  // PUR disabled
          ret |=
              RegisterSetBits(handle[brd], 0x1080 + (i << 8), 26, 26, 0, WDcfg);
          ret |=
              RegisterSetBits(handle[brd], 0x1080 + (i << 8), 7, 7, 0, WDcfg);
          // ret |= CAEN_DGTZ_WriteRegister(handle[brd], 0x107c + (i<<8),
          // 0xFFFF);
        }
        ...
        // Set the Pre-Trigger size (actually it is a post-trigger for the STD
        // FW)
        ret |= CAEN_DGTZ_SetPostTriggerSize(
            handle[brd], 100 * (WDcfg.RecordLength - WDcfg.PreTrigger) /
                             WDcfg.RecordLength);  // expressed in percent of
                                                   // the record length

        ret |= CAEN_DGTZ_SetChannelSelfTrigger(
            handle[brd], CAEN_DGTZ_TRGMODE_ACQ_AND_EXTOUT, (1 << i));
        if (WDcfg.PulsePolarity[brd][i] == 0) {
          ret |= CAEN_DGTZ_SetChannelTriggerThreshold(
              handle[brd], i,
              WDcfg.ZeroVoltLevel[brd][i] + WDcfg.TrgThreshold[brd][i]);
          ret |= CAEN_DGTZ_SetTriggerPolarity(handle[brd], i,
...
int SetVirtualProbes(int brd, Config_t WDcfg, const int handle[])
{
  ...
  if (WDcfg.WaveformProcessor) {
    ret |= RegisterSetBits(handle[brd], 0x8000, 12, 13, 0,
                           WDcfg);  // set trace0 = input
    ret |= RegisterSetBits(handle[brd], 0x8000, 11, 11, 0,
                           WDcfg);  // unset dual trace
    return 0;
  }

  // Analog Probe 0
  if ((TraceSet[0] >= 0) && (TraceSet[0] < 4) &&
      (TraceNames[0][TraceSet[0]][0] != '-')) {
    if (WDcfg.DigitizerModel != 5000)
      ret |= RegisterSetBits(handle[brd], 0x8000, 12, 13, TraceSet[0] & 0x3,
                             WDcfg);
    else
      ret |=
          RegisterSetBits(handle[brd], 0x800C, 0, 1, TraceSet[0] & 0x3, WDcfg);
  }

  // Analog Probe 1
  if (WDcfg.DigitizerModel != 5000) {
    if ((TraceSet[1] >= 0) && (TraceSet[0] < 4) && (TraceSet[1] < 4) &&
        (TraceNames[1][TraceSet[1]][0] != '-')) {
      RegisterSetBits(handle[brd], 0x8000, 11, 11, 1, WDcfg);  // set dual trace
      if ((WDcfg.DppType == DPP_PHA_724) || (WDcfg.DppType == DPP_nPHA_724))
      ...
      /*else if ((WDcfg.DppType == DPP_PSD_751) && (WDcfg.FWrev >= 8))
              ret |= RegisterSetBits(handle[brd], 0x8000, 12, 13, TraceSet[1] &
         0x3);*/
      else
    ...
    if ((TraceSet[1] >= 0) && (TraceSet[0] < 4) && (TraceSet[1] < 4) &&
        (TraceNames[1][TraceSet[1]][0] != '-')) {
      RegisterSetBits(handle[brd], 0x800C, 9, 9, 1, WDcfg);  // set dual trace
      ret |=
          RegisterSetBits(handle[brd], 0x800C, 2, 3, TraceSet[1] & 0x3, WDcfg);
    } else {
      ret |= RegisterSetBits(handle[brd], 0x800C, 9, 9, 0,
                             WDcfg);  // unset dual trace
    }
  }
  ...
  if (WDcfg.DppType == DPP_PSD_751)
    ret |=
        RegisterSetBits(handle[brd], 0x8000, 26, 26, 0,
                        WDcfg);  // digital traces 3 and 4 disabled by default
  ...
  if ((TraceSet[3] >= 0) && (TraceNames[3][TraceSet[3]][0] != '-')) {
    if (WDcfg.DppType == DPP_PSD_730) {
      ...
      ret |= RegisterSetBits(handle[brd], 0x8000, 20, 22, TraceSet[3] & 0xF,
                             WDcfg);
      ret |= RegisterSetBits(handle[brd], 0x8000, 26, 26, 1, WDcfg);
    }
  }
                  ...
                  char TraceNames[][MAX_NTRSETS][20])
{
  ...
  for (i = 0; i < MAX_NTRACES; i++) {
    for (j = 0; j < MAX_NTRSETS; j++) strcpy(TraceNames[i][j], "-");
  }
