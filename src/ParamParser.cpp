...
float GetTime(FILE *f_ini, const char *tu)
{
  ...
  if (streq(str, "ps"))
    ns = timev * 1e-3;
  else if (streq(str, "ns"))
    ns = timev;
  else if (streq(str, "us"))
    ns = timev * 1e3;
  else if (streq(str, "ms"))
    ns = timev * 1e6;
  else if (streq(str, "s"))
    ns = timev * 1e9;
  else if (streq(str, "m"))
    ...
    fseek(
        f_ini, fp,
        SEEK_SET);  // move pointer back to beginning of "str" and use it again for next parsing
    return (float)
        timev;  // no time unit specified in the config file; assuming equal to the requested one
  }
  ...
  if (streq(tu, "ps"))
    ...
    return (float)(ns / 60e9);
  else if (streq(tu, "h"))
    return (float)(ns / 3600e9);
  else if (streq(tu, "d"))
    return (float)(ns / (24 * 3600e9));
  else
    return (float)timev;
}
...
int GetHnbin(FILE *f_ini)
{
  ...
  if (streq(str, "1K"))
    ret = 1024;
  else if (streq(str, "2K"))
    ret = 2048;
  else if (streq(str, "4K"))
    ret = 4096;
  else if (streq(str, "8K"))
    ret = 8192;
  else if (streq(str, "16K"))
    ret = 16384;
  else if (streq(str, "32K"))
    ret = 32768;
  else {
    sscanf(str, "%d", &ret);
...
// Return:		0=OK, -1=error
// ---------------------------------------------------------------------------------------------------------
int ParseConfigFile(FILE *f_ini, Config_t *WDcfg, SysVars_t &SysVars)
{
  ...
  if (load_default) {
    ...
    for (b = 0; b < MAX_NBRD; b++) {
      for (i = 0; i < MAX_NCH; i++) {
        ...
        WDcfg->ChargeSensitivity[b][i] =
            -1;  // if not set by the user, this is calculated from the EnergyGain
  ...
  if (fcal != nullptr) {
    ...
    while (!feof(fcal)) {
      ...
      if ((r == 3) && (br >= 0) && (br < MAX_NBRD)) {
        if (ch == -1) {
          for (i = 0; i < MAX_NCH; i++) WDcfg->ZeroVoltLevel[br][i] = bz;
  ...
  */
  ...
  while (!feof(f_ini)) {
    ...
    if (streq(str, "AcquisitionMode")) {
      ...
      if (streq(str1, "LIST"))
        WDcfg->AcquisitionMode = ACQ_MODE_LIST;
      else if (streq(str1, "MIXED"))
        WDcfg->AcquisitionMode = ACQ_MODE_MIXED;
      else if (streq(str1, "OFF_LINE"))
        WDcfg->AcquisitionMode = ACQ_MODE_OFFLINE;
      else if (streq(str1, "EMULATOR"))
        WDcfg->AcquisitionMode = ACQ_MODE_EMULATOR_MIXED;
      else if (streq(str1, "EMULATOR_MIXED"))
        WDcfg->AcquisitionMode = ACQ_MODE_EMULATOR_MIXED;
    ...
    if (streq(str, "StartMode")) {
      fscanf(f_ini, "%s", str1);
      if (streq(str1, "INDEP_SW"))
        WDcfg->StartMode = START_MODE_INDEP_SW;
      else if (streq(str1, "SYNCIN_1ST_SW"))
        WDcfg->StartMode = START_MODE_SYNCIN_1ST_SW;
      else if (streq(str1, "SYNCIN_1ST_HW"))
        WDcfg->StartMode = START_MODE_SYNCIN_1ST_HW;
      else if (streq(str1, "TRGIN_1ST_SW"))
    ...
    if (streq(str, "TrginMode")) {
      ...
      if (streq(str1, "DISABLED"))
        WDcfg->TrginMode = TRGIN_MODE_DISABLED;
      else if ((streq(str1, "COMMON_TRG")) || (streq(str1, "EXTTRG_ONLY")))
        WDcfg->TrginMode = TRGIN_MODE_GLOBAL_TRG;
      else if ((streq(str1, "EXTTRG_START")) ||  // discontinued
               (streq(str1, "COMMON_TRG_START")))
        WDcfg->TrginMode = TRGIN_MODE_GLOBAL_TRG;  // discontinued
      else if (streq(str1, "VETO"))
        WDcfg->TrginMode = TRGIN_MODE_VETO;
      else if (streq(str1, "GATE"))
    ...
    if (streq(str, "SyncinMode")) {
      fscanf(f_ini, "%s", str1);
      if (streq(str1, "DISABLED"))
        WDcfg->SyncinMode = SYNCIN_MODE_DISABLED;
      else if (streq(str1, "TSTAMP_RESET"))
        WDcfg->SyncinMode = SYNCIN_MODE_TSTAMP_RESET;
    ...
    if (streq(str, "TrgoutMode")) {
      fscanf(f_ini, "%s", str1);
      if (streq(str1, "DISABLED"))
        WDcfg->TrgoutMode = TRGOUT_MODE_DISABLED;
      else if (streq(str1, "CHANNEL_TRIGGERS"))
        WDcfg->TrgoutMode = TRGOUT_MODE_CH_TRG;
      else if (streq(str1, "PROP_TRGIN"))
        WDcfg->TrgoutMode = TRGOUT_MODE_PROP_TRGIN;
      else if (streq(str1, "SYNC_OUT"))
        WDcfg->TrgoutMode = TRGOUT_MODE_SYNC_OUT;
      else if (streq(str1, "SQR_WAVE_1KHZ"))
        WDcfg->TrgoutMode = TRGOUT_MODE_SQR_1KHZ;
      else if (streq(str1, "PULSES_1KHZ"))
        WDcfg->TrgoutMode = TRGOUT_MODE_PLS_1KHZ;
      else if (streq(str1, "SQR_WAVE_10KHZ"))
        WDcfg->TrgoutMode = TRGOUT_MODE_SQR_10KHZ;
      else if (streq(str1, "PULSES_10KHZ"))
        WDcfg->TrgoutMode = TRGOUT_MODE_PLS_10KHZ;
      else if (streq(str1, "CLOCK"))
        WDcfg->TrgoutMode = TRGOUT_CLOCK;
    ...
    if (streq(str, "CoincMode")) {
      fscanf(f_ini, "%s", str1);
      if (streq(str1, "DISABLED"))
        WDcfg->CoincMode = COINC_DISABLED;
      else if (streq(str1, "0"))
        WDcfg->CoincMode =
            COINC_DISABLED;  // for compatibility with old settings
      else if (streq(str1, "MAJORITY"))
        WDcfg->CoincMode = COINC_MAJORITY;
      else if (streq(str1, "MINORITY"))
        WDcfg->CoincMode = COINC_MINORITY;
      else if (streq(str1, "COUPLES"))
        WDcfg->CoincMode = COINC_PAIRED_AND;  // back compatibility
      else if (streq(str1, "PAIRED_AND"))
        WDcfg->CoincMode = COINC_PAIRED_AND;
      else if (streq(str1, "ANTI_PAIRED_AND"))
        WDcfg->CoincMode = COINC_PAIRED_NAND;
      else if (streq(str1, "PAIRED_NAND"))
        WDcfg->CoincMode = COINC_PAIRED_NAND;
      else if (streq(str1, "PAIRED_OR"))
        WDcfg->CoincMode = COINC_PAIRED_OR;
      else if (streq(str1, "CH0_AND_ANY"))
        WDcfg->CoincMode = COINC_CH0_AND_ANY;
      else if (streq(str1, "CH0_NAND_ANY"))
        WDcfg->CoincMode = COINC_CH0_NAND_ANY;
      else if (streq(str1, "CH0_TO_ALL"))
        WDcfg->CoincMode = COINC_CH0_TO_ALL;
      else if (streq(str1, "AND_ALL"))
        WDcfg->CoincMode = COINC_AND_ALL;
    ...
    if (streq(str, "DiscrMode")) {
      fscanf(f_ini, "%s", str1);
      if (streq(str1, "DISABLED"))
        SetChannelParam(WDcfg->DiscrMode, DISCR_MODE_DISABLED);
      else if (streq(str1, "-1"))
        SetChannelParam(WDcfg->DiscrMode, -1);
      else if (streq(str1, "LED_PSD"))
        SetChannelParam(WDcfg->DiscrMode, DISCR_MODE_LED_PSD);
      else if (streq(str1, "RCCR2_PHA"))
        SetChannelParam(WDcfg->DiscrMode, DISCR_MODE_RCCR2_PHA);
      else if (streq(str1, "RCCR2"))
        SetChannelParam(WDcfg->DiscrMode, DISCR_MODE_RCCR2_PHA);
      else if (streq(str1, "0"))
        SetChannelParam(WDcfg->DiscrMode, 0);
      else if (streq(str1, "CFD_PSD"))
        SetChannelParam(WDcfg->DiscrMode, DISCR_MODE_CFD_PSD);
      else if (streq(str1, "CFD"))
        SetChannelParam(WDcfg->DiscrMode, DISCR_MODE_CFD_PSD);
      else if (streq(str1, "1"))
        SetChannelParam(WDcfg->DiscrMode, 1);
      else if (streq(str1, "CFD_PHA"))
        SetChannelParam(WDcfg->DiscrMode, DISCR_MODE_CFD_PHA);
      else if (streq(str1, "2"))
        SetChannelParam(WDcfg->DiscrMode, 2);
      else if (streq(str1, "LED_PHA"))
        SetChannelParam(WDcfg->DiscrMode, DISCR_MODE_LED_PHA);
      else if (streq(str1, "LED"))
        SetChannelParam(WDcfg->DiscrMode, DISCR_MODE_LED_PHA);
    ...
    if (streq(str, "InputFileType")) {
      fscanf(f_ini, "%s", str1);
      if (streq(str1, "RAW"))
        WDcfg->InputFileType = INPUTFILE_RAW;
      else if (streq(str1, "BINARY_LIST"))
        WDcfg->InputFileType = INPUTFILE_BINARY_LIST;
    ...
    if (streq(str, "HistoOutputFormat")) {
      fscanf(f_ini, "%s", str1);
      if (streq(str1, "ASCII_1COL"))
        WDcfg->HistoOutputFormat = HISTO_FILE_FORMAT_1COL;
      else if (streq(str1, "ASCII_2COL"))
    ...
    if (streq(str, "ScatterPlotMode")) {
      fscanf(f_ini, "%s", str1);
      if (streq(str1, "PSD_HORIZONTAL"))
        WDcfg->ScatterPlotMode = SCATTER_PSD_HORIZONTAL;
      else if (streq(str1, "PSD_DIAGONAL"))
        WDcfg->ScatterPlotMode = SCATTER_PSD_DIAGONAL;
    ...
    if (streq(str, "EventBuildMode")) {
      ...
      if (streq(str1, "0"))
        WDcfg->BuildMode = EVBUILD_NONE;
      else if (streq(str1, "NONE"))
        WDcfg->BuildMode = EVBUILD_NONE;
      else if (streq(str1, "1"))
        WDcfg->BuildMode = EVBUILD_CHREF_AND_ANYOTHER;
      else if (streq(str1, "CHREF_AND_ANYOTHER"))
        WDcfg->BuildMode = EVBUILD_CHREF_AND_ANYOTHER;
      else if (streq(str1, "PAIRED_AND"))
        WDcfg->BuildMode = EVBUILD_PAIRED_AND;
    ...
    if (streq(str, "EnergyCoarseGain")) {
      ...
      if (streq(str1, "1/16"))
        cg = 0.0625;
      else if (streq(str1, "1/8"))
        cg = 0.125;
      else if (streq(str1, "1/4"))
        cg = 0.25;
      else if (streq(str1, "1/2"))
        cg = 0.5;
      else
        sscanf(str1, "%f", &cg);
    ...
    if (streq(str, "OutFileTimeStampUnit")) {
      fscanf(f_ini, "%s", str1);
      if (isdigit(str1[0]))
        sscanf(str1, "%d", &WDcfg->OutFileTimeStampUnit);
      else if (streq(str1, "ps"))
        WDcfg->OutFileTimeStampUnit = 0;
      else if (streq(str1, "ns"))
        WDcfg->OutFileTimeStampUnit = 1;
      else if (streq(str1, "us"))
        WDcfg->OutFileTimeStampUnit = 2;
      else if (streq(str1, "ms"))
        WDcfg->OutFileTimeStampUnit = 3;
    ...
    if (streq(str, "RunDescription")) {
      ...
      if (i < (int)strlen(str1)) sprintf(WDcfg->RunDescription, "%s", &str1[i]);
    }
    ...
    if (streq(str, "PulsePolarity")) {
      fscanf(f_ini, "%s", str1);
      if (streq(str1, "NEGATIVE"))
        SetChannelParam(WDcfg->PulsePolarity, CAEN_DGTZ_PulsePolarityNegative);
    ...
    if (streq(str, "InputDataFileName"))
      fscanf(f_ini, "%s", WDcfg->InputDataFileName);
    if (streq(str, "DataFilePath")) fscanf(f_ini, "%s", WDcfg->DataFilePath);
    if (streq(str, "InputDataFilePath"))
      fscanf(f_ini, "%s", WDcfg->InputDataFilePath);
    if (streq(str, "ZCcalibFileName"))
      fscanf(f_ini, "%s", SysVars.ZCcalibrFileName);
    ...
    if (streq(str, "HeaderInListFiles"))
      WDcfg->HeaderInListFiles = GetInt(f_ini);
    if (streq(str, "ConfirmFileOverwrite"))
      WDcfg->ConfirmFileOverwrite = GetInt(f_ini);
    if (streq(str, "FlagsInListFiles")) WDcfg->FlagsInListFiles = GetHex(f_ini);
    if (streq(str, "AutoRunNumber"))
      WDcfg->AutoRunNumber =
          GetInt(f_ini);  // deprecated (use "RunNumber AUTO")
    if (streq(str, "StatUpdateTime"))
      WDcfg->StatUpdateTime = (int)GetTime(f_ini, "ms");
    ...
    if (streq(str, "StopOnLiveTime"))
      WDcfg->StopOnLiveTime = (int)GetTime(f_ini, "ms");
    if (streq(str, "StopOnTotalEvents"))
      WDcfg->StopOnTotalEvents = GetInt(f_ini);
    if (streq(str, "StopOnTimeEvents")) WDcfg->StopOnTimeEvents = GetInt(f_ini);
    if (streq(str, "StopOnEnergyEvents"))
      WDcfg->StopOnEnergyEvents = GetInt(f_ini);
    ...
    if (streq(str, "EnableEnergyFilter"))
      WDcfg->EnableEnergyFilter = GetInt(f_ini);
    ...
    if (streq(str, "AddBackFullScale"))
      WDcfg->AddBackFullScale = GetFloat(f_ini);
    if (streq(str, "TimeCorrelWindow"))
      WDcfg->TimeCorrelWindow = GetTime(f_ini, "ns");
    if (streq(str, "WaveformProcessor"))
      WDcfg->WaveformProcessor = GetInt(f_ini);
    ...
    if (streq(str, "CoincWindow"))
      WDcfg->CoincWindow = (int)GetTime(f_ini, "ns");
    if (streq(str, "MajorityLevel")) WDcfg->MajorityLevel = GetInt(f_ini);
    if (streq(str, "EnableInput"))
      SetChannelParam(WDcfg->EnableInput, GetInt(f_ini));
    if (streq(str, "DCoffset"))
      SetChannelParam(WDcfg->DCoffset,
                      (int)((GetFloat(f_ini) + 50) * 65535 / 100));
    if (streq(str, "ACcoupling"))
      SetChannelParam(WDcfg->ACcoupling, GetInt(f_ini));
    if (streq(str, "ACPZcomp")) SetChannelParam(WDcfg->ACPZcomp, GetInt(f_ini));
    if (streq(str, "HV_Vset"))
      SetChannelParamFloat(WDcfg->HV_Vset, GetFloat(f_ini));
    if (streq(str, "HV_Iset"))
      SetChannelParamFloat(WDcfg->HV_Iset, GetFloat(f_ini));
    if (streq(str, "HV_RampUp"))
      SetChannelParamFloat(WDcfg->HV_RampUp, GetFloat(f_ini));
    if (streq(str, "HV_RampDown"))
      SetChannelParamFloat(WDcfg->HV_RampDown, GetFloat(f_ini));
    if (streq(str, "BaselineDCoffset"))
      SetChannelParamFloat(WDcfg->BaselineDCoffset, GetFloat(f_ini));
    if (streq(str, "TriggerThreshold"))
      SetChannelParam(WDcfg->TrgThreshold, GetInt(f_ini));
    if (streq(str, "NSBaseline"))
      SetChannelParam(WDcfg->NsBaseline, GetInt(f_ini));
    if (streq(str, "TrapNSBaseline"))
      SetChannelParam(WDcfg->TrapNSBaseline, GetInt(f_ini));
    if (streq(str, "FixedBaseline"))
      SetChannelParam(WDcfg->FixedBaseline, GetInt(f_ini));
    if (streq(str, "PreGate"))
      SetChannelParam(WDcfg->PreGate, (int)GetTime(f_ini, "ns"));
    if (streq(str, "ChargeSensitivity"))
      SetChannelParam(WDcfg->ChargeSensitivity, GetInt(f_ini));
    if (streq(str, "ChargeLLD"))
      SetChannelParam(WDcfg->ChargeLLD, GetInt(f_ini));
    if (streq(str, "InputDynamicRange"))
      SetChannelParam(WDcfg->InputDynamicRange, GetInt(f_ini));
    if (streq(str, "ZeroVoltLevel"))
      SetChannelParam(WDcfg->ZeroVoltLevel, GetInt(f_ini));
    if (streq(str, "EnableZCcalibr"))
      SetChannelParam(WDcfg->EnableZCcalibr, GetInt(f_ini));
    if (streq(str, "GateWidth"))
      SetChannelParam(WDcfg->GateWidth, (int)GetTime(f_ini, "ns"));
    if (streq(str, "ShortGateWidth"))
      SetChannelParam(WDcfg->ShortGateWidth, (int)GetTime(f_ini, "ns"));
    if (streq(str, "EnablePedestal"))
      SetChannelParam(WDcfg->EnablePedestal, GetInt(f_ini));
    if (streq(str, "TrapRiseTime"))
      SetChannelParam(WDcfg->TrapRiseTime, (int)GetTime(f_ini, "ns"));
    if (streq(str, "TrapFlatTop"))
      SetChannelParam(WDcfg->TrapFlatTop, (int)GetTime(f_ini, "ns"));
    if (streq(str, "TrapPoleZero"))
      SetChannelParam(WDcfg->TrapPoleZero, (int)GetTime(f_ini, "ns"));
    if (streq(str, "PeakingTime"))
      SetChannelParam(WDcfg->PeakingTime, (int)GetTime(f_ini, "ns"));
    if (streq(str, "NSPeak")) SetChannelParam(WDcfg->NSPeak, GetInt(f_ini));
    if (streq(str, "PeakHoldOff"))
      SetChannelParam(WDcfg->PeakHoldOff, (int)GetTime(f_ini, "ns"));
    if (streq(str, "Decimation"))
      SetChannelParam(WDcfg->Decimation, GetInt(f_ini));
    if (streq(str, "SmoothedForCharge"))
      SetChannelParam(WDcfg->SmoothedForCharge, GetInt(f_ini));
    if (streq(str, "TTFsmoothing"))
      SetChannelParam(WDcfg->TTFsmoothing, GetInt(f_ini));
    if (streq(str, "TTFdelay"))
      SetChannelParam(WDcfg->TTFdelay, (int)GetTime(f_ini, "ns"));
    if (streq(str, "VetoWindow"))
      SetChannelfParam(WDcfg->VetoWindow, GetTime(f_ini, "ns"));
    if (streq(str, "TrapDigitalGain"))
      SetChannelParamFloat(WDcfg->TrapDigitalGain, GetFloat(f_ini));
    if (streq(str, "PileUpMode"))
      SetChannelParam(WDcfg->PileUpMode, GetInt(f_ini));
    if (streq(str, "PurGap")) SetChannelParam(WDcfg->PurGap, GetInt(f_ini));
    if (streq(str, "EnergyLCut"))
      SetChannelParamFloat(WDcfg->EnergyLCut, GetFloat(f_ini));
    if (streq(str, "EnergyUCut"))
      SetChannelParamFloat(WDcfg->EnergyUCut, GetFloat(f_ini));
    if (streq(str, "PsdLCut"))
      SetChannelParamFloat(WDcfg->PsdLCut, GetFloat(f_ini));
    if (streq(str, "PsdUCut"))
      SetChannelParamFloat(WDcfg->PsdUCut, GetFloat(f_ini));
    if (streq(str, "TstampOffset"))
      SetChannelParamFloat(WDcfg->TstampOffset, GetFloat(f_ini));
    if (streq(str, "THmin"))
      SetChannelParamFloat(WDcfg->THmin, GetTime(f_ini, "ns"));
    if (streq(str, "THmax"))
      SetChannelParamFloat(WDcfg->THmax, GetTime(f_ini, "ns"));
    if (streq(str, "EnergyFineGain"))
      SetChannelParamFloat(WDcfg->EnergyFineGain, GetFloat(f_ini));
    //if (streq(str, "EnergyOffset"))				SetChannelParamFloat(WDcfg->EnergyOffset,		GetFloat(f_ini));
    if (streq(str, "ECalibration_m"))
      SetChannelParamFloat(
          WDcfg->ECalibration_c1,
          GetFloat(f_ini));  // deprecated (use ECalibration c0 c1 c2 c3)
    if (streq(str, "ECalibration_q"))
      SetChannelParamFloat(
          WDcfg->ECalibration_c0,
          GetFloat(f_ini));  // deprecated (use ECalibration c0 c1 c2 c3)
    if (streq(str, "ECalibration_c0"))
      SetChannelParamFloat(WDcfg->ECalibration_c0, GetFloat(f_ini));
    if (streq(str, "ECalibration_c1"))
      SetChannelParamFloat(WDcfg->ECalibration_c1, GetFloat(f_ini));
    if (streq(str, "ECalibration_c2"))
      SetChannelParamFloat(WDcfg->ECalibration_c2, GetFloat(f_ini));
    if (streq(str, "ECalibration_c3"))
      SetChannelParamFloat(WDcfg->ECalibration_c3, GetFloat(f_ini));
    if (streq(str, "CFDdelay"))
      SetChannelParam(WDcfg->CFDdelay, (int)GetTime(f_ini, "ns"));
    if (streq(str, "CFDfraction"))
      SetChannelParam(WDcfg->CFDfraction, GetInt(f_ini));
    if (streq(str, "CFDinterp"))
      SetChannelParam(WDcfg->CFDinterp, GetInt(f_ini));
    if (streq(str, "PSDcutThr"))
      SetChannelParamFloat(WDcfg->PSDcutThr, GetFloat(f_ini));
    if (streq(str, "PSDcut"))
      SetChannelParamFloat(
          WDcfg->PSDcutThr,
          (float)GetInt(f_ini) /
              1024);  // deprecated (keep for backward compatibility)
    if (streq(str, "PSDcutType"))
      SetChannelParam(WDcfg->PSDcutType, GetInt(f_ini));
    if (streq(str, "EnableIPE"))
      SetChannelParam(WDcfg->EnableIPE, GetInt(f_ini));
    if (streq(str, "IPEamplitude"))
      SetChannelParam(WDcfg->IPEamplitude, GetInt(f_ini));
    if (streq(str, "IPErandom"))
      SetChannelParam(WDcfg->IPErandom, GetInt(f_ini));
    if (streq(str, "IPEaddnoise"))
      SetChannelParam(WDcfg->IPEaddnoise, GetInt(f_ini));
    if (streq(str, "IPErisetime"))
      SetChannelParam(WDcfg->IPErisetime, GetInt(f_ini));
    if (streq(str, "IPEfrequency"))
      SetChannelParam(WDcfg->IPEfrequency, GetInt(f_ini));
  ...
  if (WDcfg->DataFilePath[strlen(WDcfg->InputDataFilePath) - 1] != '/')
    sprintf(WDcfg->InputDataFilePath, "%s/", WDcfg->InputDataFilePath);
#else
  if (WDcfg->DataFilePath[strlen(WDcfg->DataFilePath) - 1] != '\\')
    sprintf(WDcfg->DataFilePath, "%s\\", WDcfg->DataFilePath);
  if (WDcfg->InputDataFilePath[strlen(WDcfg->DataFilePath) - 1] != '\\')
    sprintf(WDcfg->InputDataFilePath, "%s\\", WDcfg->InputDataFilePath);
#endif
  ...
  for (b = 0; b < MAX_NBRD; b++) {
    for (ch = 0; ch < MAX_NCH; ch++) {
      double binsize;
      binsize = ceil(1000.0 * (WDcfg->THmax[b][ch] - WDcfg->THmin[b][ch]) /
                     WDcfg->THnbin);  // in ps
      WDcfg->THmax[b][ch] =
          (float)(WDcfg->THmin[b][ch] + (binsize * WDcfg->THnbin) / 1000.0);
    }
  }
