...
//     if ((strlen(fdescr[nf]) > 0) &&
//         (fdescr[nf][strlen(fdescr[nf]) - 1] == '\n'))
...
//     }
//     while (!feof(cf)) {
//       if (fgets(buff, 1000, cf) != NULL) fputs(buff, ncf);
//     }
//     fclose(cf);
//     fclose(ncf);
//     nf++;
...
//       }
//       while (!feof(ncf)) {
//         if (fgets(buff, 1000, ncf) != NULL) fputs(buff, cf);
//       }
//       fclose(cf);
//       fclose(ncf);
//       return 1;
...
//   rt = fopen(fn, "w");
//   if (rt == NULL) return -1;
//   fprintf(rt, "%d %d %d %d %d %d %d\n", RTSET_VERSION, WDcfg.DppType, ChToPlot,
//           BrdToPlot, 1, HistoPlotType, Xunits);
//   for (i = 0; i < MAX_NTRACES; i++) fprintf(rt, "%d ", TraceSet[i]);
//   fclose(rt);
//   return 0;
// }
//
...
//     int unused;
//     if (ver == 1)
//       fscanf(rt, "%d %d %d %d %d\n", &dpptype, &ChToPlot, &BrdToPlot, &unused,
//              &HistoPlotType);
//     else
//       fscanf(rt, "%d %d %d %d %d %d\n", &dpptype, &ChToPlot, &BrdToPlot,
//              &unused, &HistoPlotType, &Xunits);
//     if (dpptype != WDcfg.DppType) return -1;
//     for (i = 0; i < MAX_NTRACES; i++) fscanf(rt, "%d ", &TraceSet[i]);
...
//     sprintf(str,
//             "Brd  Ch |   Throughput    OCR        Match%%  ZCcal-Progress%%    "
//             " TotCnt");
//   else if (StatsMode == 0)
//     sprintf(str,
//             "Brd  Ch |   Throughput    ICR        OCR      Match%%  DeadT%%    "
//             " TotCnt");
//   else if (StatsMode == 1)
//     sprintf(str,
//             "Brd  Ch |    Satur%%     Ovf%%  UnCorr%%    Busy%%   Queue%%      "
//             "  DeltaCnt");
// }
...
//   if (Stats.EvRead_rate[b][ch] > 0)
//     unmperc =
//         100 * (float)Stats.EvUncorrel_rate[b][ch] / Stats.EvRead_rate[b][ch];
...
//     sprintf(str, "%s   Stopped: Last time(s) = %-10.2f             %10lu", str,
//             (float)Stats.LatestProcTstamp[b][ch] / 1e9,
//             Stats.EvRead_cnt[b][ch]);
//   } else if (WDcfg.CalibrationRun) {
//     //                                              ECR   OCR   Match%%
//     //                                              ZCcal-Progress%% TotCnt");
//     sprintf(str, "%s %s %s    %6.2f%%        %d%%    %10lu", str, ecrs, ocrs,
//             100 * Stats.MatchingRatio[b][ch], ZCcal_GetFillingProgress(b, ch),
//             totnev);
//   } else if (StatsMode == 0) {
//     //                                                    ECR   ICR   OCR
//     //                                                    Match%% DeadT%%
//     //                                                    DeltaCnt");
//     sprintf(str, "%s %s %s %s %6.2f%% %6.2f%% %10lu", str, ecrs, icrs, ocrs,
//             100 * Stats.MatchingRatio[b][ch], 100 * Stats.DeadTime[b][ch],
//             totnev);
//   } else if (StatsMode == 1) {
//     //                                                                    Satur%%
//     //                                                                    Ovf%%
...
//     //                                                                    QueueOccup%%
//     //                                                                    EcCnt");
//     sprintf(str, "%s %6.2f%%  %6.2f%%  %6.2f%%  %6.2f%%  %6.2f%%      %10lu",
//             str, satperc, ovfperc, unmperc, 100 * Stats.BusyTime[b][ch],
//             GetQueueOccupancy(b, ch), nev);
//   }
// }
//
// // ---------------------------------------------------------------------------------------------------------
...
//                 uint32_t t1, t2;
//                 CAEN_DGTZ_ReadRegister(handle[b], 0x10A8,
//                                        &t1);  // read temp from ch0 (highest)
//                 CAEN_DGTZ_ReadRegister(handle[b],
//                                        0x10A8 + ((WDcfg.NumPhyCh - 1) << 8),
//                                        &t2);  // read temp from last ch
...
//         if (!DoRefresh)
//           printf(
//               "Plots and Logs refresh is now disabled; press 'o' for single "
//               "shots!\n");
//         break;
//
//       case 'F':
//         if ((WavePlotType == WPLOT_WAVEFORMS) &&
...
//         if ((strlen(WDcfg.RunDescription) > 0) &&
//             (WDcfg.RunDescription[strlen(WDcfg.RunDescription) - 1] == '\n'))
...
//         else
//           sprintf(ext, "txt");
//         sprintf(cmd, "start keview DataFiles\\Run%d_Ehisto_%d_%d.%s",
//                 WDcfg.RunNumber, BrdToPlot, ChToPlot, ext);
//         system(cmd);
//         break;
//
//       case 'K':
...
//             sprintf(cmd, "%s DataFiles\\Run%d_Ehisto_%d_%d.%s", cmd,
//                     WDcfg.RunNumber, BrdToPlot, ch, ext);
//         system(cmd);
//         break;
//
//       case 'l':
...
//           printf(
//               "The waveform readout is not enabled; restart acquisition to "
//               "enable [y/n]?\n");
//           if (getch() != 'y') break;
//           WavePlotType = WPLOT_WAVEFORMS;
//           ForceWaveMode = 1;
...
//           RestartAll = 1;
//           AcqRun = 0;
//         } else if ((WavePlotType != WPLOT_DISABLED) &&
//                    (WDcfg.AcquisitionMode == ACQ_MODE_MIXED)) {
//           printf(
//               "The waveform readout is enabled; restart acquisition to disable "
//               "[y/n]?\n");
//           if (getch() != 'y') break;
...
//                                         WDcfg.EnableZCcalibr[b][ch] =
//         WDcfg.EnableInput[b][ch]; ZCcal_ResetZCTables(SysVars.ZCcorr_Ncnt); }
//         else { ZCcal_SaveCorrectionTables(SysVars.ZCcalibrFileName); RestartAll
//         = 1;
//                 }
//                 break;
//                 */
...
//         printf(
//             "To select an option press the relevant key or Enter to "
//             "continue\n\n");
//         c = getch();
//         goto menu;
//         break;
...
//   int TestConnection =
//       0;  // Test connection with the board, read info then quit
//   // Other variables
//   int i, b, ch, ForceStatUpdate = 1, NoData = 1;
//   uint32_t AllocSize, TotAllocSize = 0;
//   int ExitSleep = WAIT_ON_EXIT;  // waiting time before closing the console
//                                  // (only in case of error)
//   int Wave_RefreshRate = 200;
//   uint64_t CurrentTime, ElapsedTime, PrevStatTime, PrevLogTime, PrevPlotTime,
//       PrevKeybTime, PrevAustosave;
...
//         printf(
//             " ConfigFileName = configuration file (default = "
//             "digiTES_Config.txt)\n");
//         printf(" -i : Immediate Start Acquisition (don't ask to press 's')\n");
//         printf(
//             " -l : reload previous settings for plots (channel, traces, "
//             "etc...)\n");
//         printf(" -rN : set run number = N\n");
//         printf(
//             " -fInputDataFile : Off-line run taking data from InputDataFile\n");
...
//     WDcfg.AcquisitionMode = ACQ_MODE_MIXED;  // force waweform mode
...
//   if (OvwrRunNumber >= 0) WDcfg.RunNumber = OvwrRunNumber;
...
//     if (cntlog == NULL) {
//       msg_printf(MsgLog,
//                  "ERROR: Can't open connection log file %s. Quitting...\n",
//                  filename);
//       Failure = 1;
//       goto QuitProgram;
//     }
...
//         msg_printf(MsgLog, "\nINFO: Opening digitizer n. %d through %s %d ", b,
//                    ct[WDcfg.LinkType[b]], WDcfg.LinkNum[b]);
//         if (WDcfg.LinkType[b] != 0)
//           msg_printf(MsgLog, "%d ", WDcfg.ConetNode[b]);
//         if (WDcfg.BaseAddress[b] != 0)
//           msg_printf(MsgLog, "BA=%08X ", WDcfg.BaseAddress[b]);
//         ret = CAEN_DGTZ_OpenDigitizer(
//             (CAEN_DGTZ_ConnectionType)WDcfg.LinkType[b], WDcfg.LinkNum[b],
//             WDcfg.ConetNode[b], WDcfg.BaseAddress[b], &handle[b]);
//         if (ret) {
...
//       }
//     }
//   } else if (WDcfg.AcquisitionMode ==
//              ACQ_MODE_OFFLINE) {  // Off-line run (data coming from file)
...
//   for (b = 0; b < WDcfg.NumBrd; b++)
//     for (i = WDcfg.NumAcqCh; i < MAX_NCH; i++) WDcfg.EnableInput[b][i] = 0;
//
//   if (cntlog != NULL) fclose(cntlog);
//   if (TestConnection) {  // Just tested the connection to the boards; the
//                          // program quits here
//     ExitSleep = 0;
//     goto QuitProgram;
//   }
...
//   msg_printf(
//       MsgLog,
//       "INFO: Allocated %.2f MB for readout data, histograms and counters\n",
...
//   if (!DoRefresh)
//     printf(
//         "Plots and Logs are OFF; press 'f' to enable or 'o' for a singles\n");
//
...
//     // -----------------------------------------------------------------------------------
//     if (ACQ_MODE_PLUGGED(WDcfg.AcquisitionMode) &&
//         (ContinuousTrigger || SingleTrigger)) {
//       if ((WDcfg.TrginMode == TRGIN_MODE_VETO) ||
...
//         if (SingleTrigger) printf("Single Software Trigger issued\n");
//       }
//       SingleTrigger = 0;
//     }
//
...
//       uint64_t cfgtime;
//       if (ForceStatUpdate ||
//           ((CurrentTime - PrevStatTime) > WDcfg.StatUpdateTime)) {
...
//       ClearScreen();
//       printf("Press [Space] for help\n");
//       if (WDcfg.JobStopRun > 0)
//         printf("RUN n. %d - Job %d of %d\n", WDcfg.RunNumber,
//                WDcfg.RunNumber - WDcfg.JobStartRun + 1,
//                WDcfg.JobStopRun - WDcfg.JobStartRun + 1);
//       else
...
//       if (AcqRun &&
//           ((WDcfg.StopOnTime == 0) || (Stats.AcqRealTime < WDcfg.StopOnTime))) {
//         if (Stats.RealTimeSource == REALTIME_FROM_BOARDS)
//           printf("RealTime (from boards) = %.2f s", Stats.AcqRealTime / 1000);
//         else
//           printf("RealTime (from computer) = %.2f s", Stats.AcqRealTime / 1000);
//         if (WDcfg.StopOnTime > 0)
//           printf(" - Stop Time = %.2f s (%d%%)\n",
//                  (float)WDcfg.StopOnTime / 1000,
//                  (int)(100 * Stats.AcqRealTime / WDcfg.StopOnTime));
//         else
//           printf("\n");
//       } else {
...
//
//       if (HistoPlotType != HPLOT_DISABLED)
//         PlotSelectedHisto(HistoPlotType, Xunits);
//
...
//     }
//
//     // if not running, close the while loop here (the Sleep prevents heavy CPU
//     // usage)
//     if (!AcqRun) {
//       Sleep(10);
//       continue;
...
//
//     // AutoSave histograms
//     if ((SysVars.HistoAutoSave > 0) &&
//         ((CurrentTime - PrevAustosave) > (SysVars.HistoAutoSave * 1000))) {
//       SaveAllHistograms();
...
//         (NoData ||
//          ACQ_MODE_UNPLUGGED(
//              WDcfg.AcquisitionMode))) {  // Stop command from the keyboard
//       SaveAllHistograms();
//       Stopping = 0;
//       AcqRun = 0;
...
//     for (b = 0; b < WDcfg.NumBrd; b++) {
//       for (ch = 0; ch < WDcfg.NumAcqCh; ch++) {
//         if (((WDcfg.StopOnTotalEvents > 0) &&
//              (Stats.EvRead_cnt[b][ch] >= (uint32_t)WDcfg.StopOnTotalEvents)) ||
//             ((WDcfg.StopOnTimeEvents > 0) &&
//              (Histos.TH[b][ch].H_cnt >= (uint32_t)WDcfg.StopOnTimeEvents)) ||
//             ((WDcfg.StopOnEnergyEvents > 0) &&
//              (Histos.EH[b][ch].H_cnt >= (uint32_t)WDcfg.StopOnEnergyEvents)) ||
//             ((WDcfg.StopOnTime > 0) && (Stats.LatestProcTstamp[b][ch] >=
//                                         ((uint64_t)WDcfg.StopOnTime * 1e6))) ||
//             ((WDcfg.StopOnTime > 0) &&
//              ((Stats.AcqRealTime > (float)WDcfg.StopOnTime) &&
//               (Stats.EvRead_rate[b][ch] == 0))) ||
//             ((WDcfg.StopOnLiveTime > 0) &&
//              (Stats.AcqRealTime * (1 - Stats.DeadTime[b][ch]) >=
//               (WDcfg.StopOnLiveTime / 1000)))) {
...
//               if (WDcfg.EnableInput[bb][cc] && !StopCh[bb][cc]) AllStopped = 0;
//           if (AllStopped) {
//             StopAcquisition();
//             SaveAllHistograms();
//             AcqRun = 0;
//             if (Stats.AcqStopTime < (float)WDcfg.StopOnTime)
//               Stats.AcqStopTime = (float)WDcfg.StopOnTime;
...
//       NoData = 0;
//
//       if ((WDcfg.SaveLists & SAVELIST_BUILTEVENTS) &&
...
//             continue;
//           if (StopCh[b][ch]) {  // single channel stopped: data are still
//                                 // coming, but they are discarded
//             Stats.EvRead_cnt[b][ch]--;
//             FreeWaveform(evnt[b][ch].Waveforms);
//             continue;
//           }
//
//           // Get the newest time stamp (used to calculate the real acquisition
//           // time)
...
//           // Energy & PSD Spectra
//           // ------------------------------------------------------------------------------
//           if ((evnt[b][ch].Flags & EVFLAGS_PILEUP) &&
//               !((WDcfg.DppType == DPP_PSD_730) &&
//                 (WDcfg.PileUpMode[b][ch] == 0)))
//             pur = 1;
//           if (!(pur || evnt[b][ch].Flags & EVFLAGS_SATUR)) {
//             if (evnt[b][ch].Flags & EVFLAGS_E_OVR) {
//               Ebin = 0xFFFF;
//             } else if (evnt[b][ch].Flags & EVFLAGS_E_UNR) {
//               Ebin = -1;
//             } else {
...
//                 by = (int)((evnt[b][ch].Energy -
//                             evnt[b][ch].psd * evnt[b][ch].Energy) *
//                            HISTO2D_NBINY / WDcfg.EHnbin);
//               else
...
//             if (((ch & 1) == 0) &&
//                 (NumEv[b][ch + 1])) {  // assuming deltaE is in the same channel
//                                        // pair of E (ch0=E, ch1=dE, etc...)
...
//
//           // ------------------------------------------------------------------------------
//           // Sample Histogram (NOTE: the sample histogram grows only when active
//           // on the plot)
//           // ------------------------------------------------------------------------------
//           if ((evnt[b][ch].Waveforms != NULL) &&
//               (HistoPlotType == HPLOT_SAMPLES)) {
//             for (i = 0; i < evnt[b][ch].Waveforms->Ns; i++)
//               Histo1D_AddCount(&Histos.SH[b][ch],
//                                evnt[b][ch].Waveforms->AnalogTrace[0][i]);
//           }
//
//           // ------------------------------------------------------------------------------
//           // Time Spectrum
...
//             time =
//                 (double)(evnt[b][ch].TimeStamp -
//                          PrevChTimeStamp[b][ch]);  // delta T between pulses on
//                                                    // the same channel (in ns)
//           else
//             time = (double)evnt[b][ch].TimeStamp -
//                    (double)evnt[BrdRef][ChRef].TimeStamp +
//                    ((double)evnt[b][ch].FineTimeStamp -
//                     (double)evnt[BrdRef][ChRef].FineTimeStamp) /
//                        1000;  // delta T from Ref Channel (in ns)
//
//           PrevChTimeStamp[b][ch] = evnt[b][ch].TimeStamp;
//           Tbin = (uint32_t)((time - WDcfg.THmin[b][ch]) * WDcfg.THnbin /
//                             (WDcfg.THmax[b][ch] - WDcfg.THmin[b][ch]));
//           Histo1D_AddCount(&Histos.TH[b][ch], Tbin);
//
//           // ------------------------------------------------------------------------------
//           // Multi Channel Scaler
//           // ------------------------------------------------------------------------------
//           Tbin = (uint32_t)(evnt[b][ch].TimeStamp /
//                             ((uint64_t)WDcfg.DwellTime * 1000));
...
//           if (WDcfg.SaveLists & SAVELIST_INDIVIDUAL)
...
//
//           // Waveform Plotting
//           if ((b == BrdToPlot) && (ch == ChToPlot) &&
//               (WavePlotType != WPLOT_DISABLED) &&
//               ((CurrentTime - PrevPlotTime) > Wave_RefreshRate) &&
//               (DoRefresh || DoRefreshSingle) &&
//               (evnt[b][ch].Waveforms != NULL)) {
//             char title[500];
//             if (WavePlotType == WPLOT_WAVEFORMS) {  // Waveform
//               sprintf(title, "WAVEFORMS : Board %d - Channel %d (Energy = %7d)",
//                       b, ch, evnt[b][ch].Energy);
...
//                 sprintf(title, "FFT : Board %d - Channel %d (Mean = %d)", b, ch,
//                         NmeanFFT);
...
//     AcqRun = 0;
//   }
//   msg_printf(MsgLog, "INFO: Closing Run %d.\n\n", WDcfg.RunNumber);
//   DestroyHistograms();
//   CloseReadout();
//   CloseOutputDataFiles();
//   CloseFFT();
//   if ((WDcfg.JobStopRun > WDcfg.JobStartRun) &&
//       (WDcfg.RunNumber < WDcfg.JobStopRun)) {
//     char c = 'n';
